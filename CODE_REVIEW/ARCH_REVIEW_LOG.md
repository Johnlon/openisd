...
## 2026-07-21 19:13:39 UTC — arch review of 43904363c (feat: selectable port end correction on every skin (WinISD parity))
- Mechanical (engine architecture.test.ts): PASS — `npx vitest run --project engine packages/engine/test/architecture.test.ts` → 3/3 tests green, no boundary breaks.
- Full unit gate (`npm run test:unit`): PASS — 25 files / 239 tests, incl. `engine/test/golden.test.ts` (6/6) and the new `alignments.test.ts` "selectable port end correction" describe block (both `tuningFromLength(Vb,L,Sp,0.732) === tuningFromLength(Vb,L,Sp)` and `ventLength(Vb,fb,Sp,0.732) === ventLength(Vb,fb,Sp)` assertions pass) — confirms the default-param no-op claim and byte-identical goldens empirically, not just by inspection.
- No-op verified by code inspection too: `packages/engine/src/alignments.ts:69,82` — `endCorrection: number = END_CORRECTION` (same constant, `packages/engine/src/constants.ts:20`), and every internal engine call site that matters (`packages/engine/src/circuit.ts:105` box-tuning port impedance) consumes the already-resolved `P.Leff` rather than re-deriving it, so no missed call site.
- Layering/import-cycle: clean. `packages/ui/src/store.ts:2` and `packages/ui/src/shells/original/OriginalShell.vue:27` both import `END_CORRECTION` from `@openisd/engine` (pre-existing import, unchanged direction — UI→engine only). `alignments.ts`'s only change is an additional primitive-typed default param; no DOM/Vue reference introduced in engine source.
- Persisted-state fallback: verified end-to-end. `packages/ui/src/store.ts:12-19` (`P_DEFAULTS.endCorrection = 0.732`) seeds `state.P` before any restore; `packages/ui/src/App.vue:32` (`if (o.P) Object.assign(state.P, o.P)`) merges an older saved project's `P` bag that lacks the `endCorrection` key without touching the pre-seeded default (Object.assign only copies keys present on the source); `resetProjectToGround` (`store.ts:265`) does the same merge from the ground fingerprint. `syncedP`'s own `(state.P.endCorrection ?? END_CORRECTION)` (`store.ts:187`) and `OriginalShell.vue:107` are a second, redundant safety net for the case `endCorrection` is present but `null`/`undefined` some other way. Clean fallback, no gap.
- [SEVERITY: low] `CONTRACT.md:193-194` — CONTRACT.md drift: `ventLength(Vb, Fb, Sp)` / `tuningFromLength(Vb, Leff, Sp)` rows still show the pre-change 3-arg signature; the commit did not update the contract doc for the new optional 4th `endCorrection` param.
  why: CONTRACT.md is meant to be the stable/current engine API surface; a signature change (even additive/back-compat) left undocumented means a reader of CONTRACT.md doesn't know the param exists.
  suggested fix: update both rows to `ventLength(Vb, Fb, Sp, endCorrection?)` / `tuningFromLength(Vb, Leff, Sp, endCorrection?)` with a one-line note that it defaults to 0.732 (no-op).
- [SEVERITY: medium] `packages/ui/src/shells/original/OriginalShell.vue:632-637` vs `mock/index.html:477` — structural/placement divergence in the Vents pane. The mock pairs "End Correction" in the SAME field-row as "Vent diameter" (left column, row 2: `Vent diameter | End Correction`, `mock/index.html:476-477`). OriginalShell instead inserts it as a new, solitary field-row underneath the existing Vent diameter/Vent length row (`OriginalShell.vue:630-638`), changing the two-column grid the mock uses throughout this tab.
  why: 3a mandate — the Original skin must be structurally identical to the mock; the only sanctioned divergence is fake-data→store/engine substitution, not re-laying-out fields.
  suggested fix: move the End-Correction `<select>` into the existing Vent-diameter field-row as a second field, matching `mock/index.html:476-477` exactly, and drop the new standalone field-row.
- [SEVERITY: medium] `packages/ui/src/shells/original/OriginalShell.vue:633-636` vs `mock/index.html:477` — option content/order divergence. Mock: bare numeric option text, order `0.732, 0.613, 0.849` (`<option>0.732</option><option>0.613</option><option>0.849</option>`). Shell: descriptive labels "Two Free Ends (0.613)" / "One Flanged End (0.732)" / "Two Flanged Ends (0.849)", ascending order 0.613/0.732/0.849 (`EC_OPTIONS`, `OriginalShell.vue:122-127`).
  why: mock fidelity requires identical option text/order, not a friendlier rewrite; this is exactly the kind of "simplified/renamed markup" 3a calls out.
  suggested fix: either match the mock's bare-number, 0.732-first option markup exactly, or if the descriptive labels are an intentional, wanted improvement, get that logged as a sanctioned mock divergence in BACKLOG.md rather than left as a silent drift (currently untracked).
- [SEVERITY: medium] `packages/ui/src/shells/original/OriginalShell.vue:634` (`style="width:190px"`) vs `mock/style.css:296-299` — chrome divergence. Every other `.field select`/input in the mock (and the rest of this Vents pane) is the default 90px (`.field input[type=text], .field input[type=number], .field select { width: 90px; }`); this is the only field in the pane with a hardcoded inline `width:190px`, and also the only field carrying a `class="field entered"` on a `<select>` where the mock's equivalent (`mock/index.html:477`) has plain `class="field"` (no entered/calculated provenance styling — WinISD end-correction is a preset picker, not a green "user entered" numeric value).
  why: unmatched sizing/coloring breaks the pane's visual consistency with the mock (3a: "different palette, spacing… chrome vs mock/style.css").
  suggested fix: drop the inline width (let the 90px default apply, widening only if the mock itself is amended first) and drop `entered` from the wrapping `<div>` to match `mock/index.html:477`'s plain `.field`.
- [SEVERITY: high] `packages/ui/src/shells/original/OriginalShell.vue:629` vs `mock/index.html:482` — pre-existing (not introduced by this commit, but in the touched region) divergence: mock's "Vent length" is a CALCULATED readonly readout (`class="calculated greyed"`, driven from target Fb, in the right column) while OriginalShell renders it as a directly user-entered `NumInput` (`class="field entered"`, left column, paired with Vent diameter). This is a real WinISD-workflow difference (target-Fb-driven auto-tune vs manual length entry), not cosmetic. No matching BACKLOG.md item currently tracks it (checked: BACKLOG.md only tracks the NumInput spinner-dp bug and the (now-removed, already-done) "add End Correction select" item for this pane, not the entered-vs-calculated Vent-length mismatch).
  why: 3a — this is exactly the class of "different provenance/structure" divergence the mandate requires surfacing even when pre-existing/deferred, and it's currently untracked so it will not surface again unless someone re-derives it from the mock by hand.
  suggested fix: either re-derive Vent length as a calculated field from target Fb + Vb + Sp (matching mock semantics) or, if entered-length is an intentional OpenISD design choice (vent length as an independent design input, Fb as the derived readout — which IS how this codebase's `tuningFromLength`/`ventLength` split already models it), record that divergence explicitly in BACKLOG.md as a sanctioned deviation from the mock rather than leaving it silent.
- RCA_REQUIRED: no — single commit 43904363c, commit message has no bug/regression/broke/revert/hotfix/fixup/"work around" language; `git log --format='%s%n%b' 43904363c~1..43904363c` shows only the feature description with a stated red→green TDD sequence (engine unit test + browser test written first, both later green); no dated 2026-07-21 entry in `CODE_REVIEW/POST_MORTEM.md` (its two entries are 2026-07-19 and 2026-07-20, both before this commit) — no mid-build bug in range.

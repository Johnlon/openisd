...
for filepath in files_to_check:
    try:
        ...
        for field, mapped in field_map.items():
            content = re.sub(rf'\.{mapped}\.value', rf'.{mapped}.get().value', content)
            content = re.sub(rf'\.{mapped}\.state', rf'.{mapped}.get().state', content)
            
            # and if the file has `.empty()` for Driver, fix it. But we don't have engine in scope in all tests.
            # I will just import blankDriverRecord and engine where needed manually.

...
- HTTP LINKS ARE THE DEFAULT: serve every file/doc you mention through the local agentutils HTTPD on port 8000.
  ...
  - ONLY translate paths that are mounted in the server. Mounted paths like `file://wsl.localhost/Ubuntu-22.04/home/john/work/winisd/XXX` (excluding dotfile segments) MUST become `http://localhost:8000/winisd/XXX`.
  - NEVER route unmounted paths or dotfile segments (e.g. `.claude/`, `.agents/`) through the HTTP server (the server refuses them with 403).
- Fallback when the file is outside every mount or in a dot-directory: use Windows-compatible UNC syntax (e.g., `[filename](file://wsl.localhost/Ubuntu-22.04/absolute/path/to/file#L123-145)`).

...
`test/persistence/bundled-index-artifacts.test.ts`, which fails on any duplicate uuid or path
in the tracked catalogue indexes.

## The browser suite's catalogue

The suite's vite serves a six-device catalogue — `packages/ui/test/fixtures/test-bundle-paths.json`,
cut from the tracked `packages/ui/public/` indexes by `scripts/test-bundle.mjs` — not the whole
corpus. A spec that names a bundled device names one of those six; a spec that needs another
adds its path to the fixture list (the artifact gate proves it exists).

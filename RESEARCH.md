...
This is the distilled version. The evidence (probe runs, disassembly, arithmetic) stays in the
source documents linked from each section.
...
| [`docs/research/VENTED_ALIGNMENT_FORMULAS.md`](docs/research/VENTED_ALIGNMENT_FORMULAS.md) | WinISD's five vented alignments (QB3, BB4, EBS3, EBS6, C4), recovered from the binary and matched to 35 captures within 1e-14. Implemented in `engine/boxDesign.ts`. |

### Circuits and schematics

| Source                                                                                                  | What it is                                                                                  |
|---------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------|
| [`winisd_research/references/yanapack_circuits/`](../winisd_research/references/yanapack_circuits/)     | SPICE-style netlists (`driver.cir`, `closed_cab.cir`, `surround-27-vented.cir`) for yanapack, an independent numerical oracle. |
...
| [`docs/research/WINISD_PARITY.md`](docs/research/WINISD_PARITY.md) §9                                   | WinISD's circuit model against the full gyrator model (Le in or out of the acoustic path).   |
...
| [`docs/design/WINISD_SCHEMA.md`](docs/design/WINISD_SCHEMA.md)                   | `.wdr`/`.wpr` byte format and the 22 consistency relations.                        |
| [`docs/research/WINISD_PARITY.md`](docs/research/WINISD_PARITY.md)               | Field-by-field UI parity against the WinISD screenshots. Dated 2026-08-13; the gap list supersedes its status marks. |
...
`winisd_research/` is a separate repository and is git-ignored by the parent workspace.
Its documents have no history in this repository.
...
> recomputed and never questioned.

- This is constraint propagation to a fixpoint. It is not field precedence and not recency.
- The relations are the 22 rows of `WINISD_SCHEMA.md` §4: 15 read from `winisd.exe`'s strings, 7 inferred.
...
- **Source of the values:**
  ...
  - A driver's stored `c`/`roo` win when present.
...
- **OpenISD's physical model** (CIPM-2007) differs from WinISD's by 8.3 ppm on ρ and 4.1 ppm on c at the reference point. The cause is WinISD's shorter constants.
...
| Force flat response has no boost ceiling; it pins the reference level at about 95 dB of implied gain.        | FINDING-010    |
| Box-tab Volume and Tuning do not cascade on typing; coupling differs by box type and follows the chamber.   | FINDING-004    |
...
| The `.wdr` writer emits UTF-8, and WinISD's own reader cannot read non-ASCII back.                           | PROBE_FINDINGS §"Comment field" |
...
Checked against OpenISD's code and tests on 2026-09-25. A row marked "not verified" has not been checked.

| WinISD bug                                                                                  | OpenISD                                                                                                              |
|---------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------|
| BUG-001/002/003/004: crashes and popup loops on an empty driver, the Box tab or a blank PR volume | A missing input leaves dependent fields N, and the sweep returns a named missing-dependency issue instead of values. |
...
| BUG-008: `Dd` unit inconsistency                                                            | Not verified. An open OpenISD bug records dimension fields stored in mm and litres.                                  |
...
- **Reading numbers back, cheapest first:**
  1. the saved `.wpr` at 15 significant digits;
  2. E/C/N state from exact pixel colour;
  3. screenshots.
- **Getting WinISD to recalculate:**
  - The only approved trigger is retyping a value as itself, spelled differently (for example `0.00050` over `0.0005`).
  - A Driver-editor edit fires the derived-field pass.
...
- **Probing both apps on one case:**
  ...
  - WinISD curves are read from plot pixels by exact colour.
  - OpenISD's are dumped from `Engine.sweep` with the parameters the app passes.
  - Example: `PROBE_W5_SEALED_20260924.md`, using the matched W5-1138SMF pair `winisd_drivers/.../openisd.json` + `winisd.wdr`.
...
- Results are cross-checked against DPC, and then against yanapack as an independent oracle.

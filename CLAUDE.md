...
## drivers-bundle.json is DERIVED — never tracked, CI uses both repos — 🔒 LOCKED (2026-07-21)

`packages/ui/src/drivers-bundle.json` is generated by `scripts/bundle-drivers.mjs`
(triggered by `predev`/`prebuild`, and by `scripts/ci.sh` before tests). Human ruling
2026-07-21 (re-stated emphatically; previously discussed): **the derived bundle is NEVER
checked in, and CI MUST have BOTH sibling repos (`openisd` + `winisd_drivers`) checked
out and regenerate the bundle from them.**

- The file is git-ignored (`.gitignore` — the pre-2026-07-21 pattern was root-anchored
  and matched nothing, which is how a tracked copy survived; fixed).
- Never `git add -f` the bundle, never restore a tracked copy "so a bare clone builds" —
  a build/test environment without the `winisd_drivers` sibling is mis-provisioned, not
  something to paper over with stale vendored data.
- If a test can't find a driver the bundle should carry, the fix is scraping that driver
  into `winisd_drivers/db` (see winisd_tools CLAUDE.md "Missing driver fixture in the
  sibling app → go scrape it"), then regenerating — never editing the bundle.

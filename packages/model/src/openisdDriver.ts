...
//
// /**
//  * Every spec field name, AS DATA — the runtime twin of `SpecField`, which exists only in the
//  * type system and so can never check a string that arrives at runtime.
//  *
//  * Declared `readonly SpecField[]`, so the COMPILER checks every name here against
//  * `keyof SpecSection`: a typo, or a field renamed in the interface, fails to compile at this
//  * declaration. That is what makes `isSpecField` a real narrowing rather than a cast wearing a
//  * function's clothes.
//  */
// const SPEC_FIELDS = [
//   'Fs', 'Re', 'Le', 'fLe', 'KLe', 'Znom', 'Qts', 'Qes',
//   'Qms', 'Vas', 'Sd', 'BL', 'Mms', 'Cms', 'Rms', 'Xmax',
//   'Xlim', 'SPL', 'Pe', 'Dd', 'EBP', 'numVC', 'VCCon', 'Dia',
//   'Vd', 'no', 'SPLmax', 'SPLmaxLF', 'USPL', 'alfaVC', 'Rt', 'Ct',
//   'gamma', 'Rme', 'Mpow', 'Mcost', 'Gloss', 'c', 'roo', 'Vcd',
//   'Hg', 'Hc', 'freq_low_hz', 'freq_high_hz', 'power_peak_W', 'weight_kg', 'Thick', 'Depth',
//   'MagDepth', 'Magnet', 'Basket', 'Outer', 'OuterX', 'OuterY', 'DVol',
// ] as const satisfies readonly SpecField[];
//
// /**
//  * THE COMPILER PROVES THE LIST IS COMPLETE, not just correct.
//  *
//  * `satisfies readonly SpecField[]` above rejects a name `SpecSection` does not declare. That
//  * alone is not enough: a field ADDED to `SpecSection` and forgotten here would leave
//  * `isSpecField` answering false for a real field, and the record's value would be dropped in
//  * silence — the same failure this whole guard exists to stop, arriving from the other side.
//  *
//  * So the missing set is computed and required to be empty. Add a field to `SpecSection` without
//  * adding it here and `Missing` becomes that field's NAME, the assignment below fails, and the
//  * error names the field. Nothing is trusted to anybody's memory.
//  */
// type MissingSpecFields = Exclude<SpecField, typeof SPEC_FIELDS[number]>;
// const _everySpecFieldIsListed: MissingSpecFields extends never ? true : MissingSpecFields = true;
// void _everySpecFieldIsListed;
//
// const SPEC_FIELD_SET: ReadonlySet<string> = new Set<string>(SPEC_FIELDS);
//
// /** Whether a runtime string is a declared spec field. A TYPE GUARD, not an assertion: a name
//  *  the record does not declare is refused here instead of being written in as a key nothing
//  *  can read back. */
// export function isSpecField(name: string): name is SpecField {
//   return SPEC_FIELD_SET.has(name);
// }
//
// /**
...
//       if (!isSpecField(key)) continue;
//       woofer[key] = { origin: 'manual', readings: { manual: { read_value: v } }, dq: [] };
//     }
//
...
//     for (const k of Object.keys(specs).filter(isSpecField)) {
//       const entry = specs[k];
//       if (!entry) continue;
...
//     }
//     const specs = this.#specs();
//     for (const key of Object.keys(specs).filter(isSpecField)) {

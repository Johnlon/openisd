...
//   humidityPct: number;
//   pressurePa: number;
//   /** Which FORMULA computes ρ/c: WinISD's own (true) or OpenISD's CIPM-2007 model (false). */
//   useWinisdAirModel: boolean;
//   /** Which environment SOURCE to use: the app-level Options dialog (true, reproducing WinISD's
//    *  behaviour of never reading a project's own stored T/RH/AP) or this project's own (false). */
...
//       // overriding the physical-model default long used in these notes). WinISD parity gives
//       // the same answer a WinISD user sees for the same environment, while the physics model is
//       // the standard CIPM-based calculation. The difference is measurable but tiny — worst case
//       // across 0-40 °C, 0-100 % RH and 95-105 kPa is about 0.003 dB SPL with no F3 shift
//       // (winisd_research FINDING-008). A user picks one or the other so the design can be
//       // checked against WinISD or against the standard physical model.
//       tempK: 293.15, humidityPct: 30, pressurePa: 101325, useWinisdAirModel: true,
...
//   pressurePa?: number;
//   /** Select WinISD's parity air model instead of OpenISD's CIPM-based physical model. */
//   useWinisdAirModel?: boolean;
//   /** When the WinISD parity model is active, take T/RH/p from the app-level Options defaults. */
//   useAppLevelAirEnvironment?: boolean;
// }
//
...
//
//   useWinisdAirModel(): boolean { return this.#record.environment.useWinisdAirModel ?? false; }
//   setUseWinisdAirModel(value: boolean): void { this.#record.environment.useWinisdAirModel = value; }
//   useAppLevelAirEnvironment(): boolean { return this.#record.environment.useAppLevelAirEnvironment ?? false; }
//   setUseAppLevelAirEnvironment(value: boolean): void { this.#record.environment.useAppLevelAirEnvironment = value; }
//
...
//       useWinisdAirModel: this.useWinisdAirModel(),
//       useAppLevelAirEnvironment: this.useAppLevelAirEnvironment(),
//     };
//   }
//
//   /**
...
//     // `tempK`/`humidityPct`/`pressurePa`/`useWinisdAirModel`/`useAppLevelAirEnvironment` are the
//     // only FIVE fields the environment snapshot declares optional — old serialised blobs can
//     // genuinely omit them, and the record adds them as the project-level air semantics grew.
//     // Every other field is required by `UiParams`, so `field()` alone type-checks for them.
//     // These five need one more step: `current[k]` is read from the live record, where the
//     // environment fields are never undefined, so this narrows `field()`'s `T | undefined` back
//     // to `T` without inventing a fallback value.
//     const requiredField = <K extends 'tempK' | 'humidityPct' | 'pressurePa' | 'useWinisdAirModel' | 'useAppLevelAirEnvironment'>(k: K)
...
//     this.setUseWinisdAirModel(requiredField('useWinisdAirModel'));
//     this.setUseAppLevelAirEnvironment(requiredField('useAppLevelAirEnvironment'));

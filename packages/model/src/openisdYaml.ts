/**
 * `openisd.yml` / `.owdr` read/write.
 *
 * READ is solid: any valid YAML matching `OpenISDDriver`'s shape parses correctly,
 * regardless of which side wrote it — YAML syntax doesn't care about key order.
 *
 * WRITE is a first cut, NOT yet verified byte-identical to the Python side's canonical
 * serialisation. The Python writer (`model_driver.py:1338-1365`, `canonical_yaml()`) has
 * its own dialect — a `_KEY_PRIORITY`-driven key sort, flow-style (`[a, b, c]`) for
 * certain keys, and it OMITS an empty `dq: []` entirely rather than writing it. None of
 * that is replicated here yet. This writer emits every field, block-style, in the order
 * `OpenISDDriver`'s constructor declares them (which mirrors MetaFile's Python field
 * order) — a REASONABLE canonical order, not a verified-identical one. Two files holding
 * the same record, one written by each language, are not guaranteed to diff clean yet.
 * Needs its own pass reading `_KEY_PRIORITY`/`_FLOW_LIST_KEYS` before that claim can be
 * made — flagged here rather than silently assumed.
 */
import { parse, stringify } from 'yaml';
import { OpenISDDriver } from './openisdDriver.js';

/** Parse `openisd.yml`/`.owdr` text into an `OpenISDDriver`. Throws on invalid YAML;
 *  does NOT yet validate the result against the schema (no runtime shape-check — trusts
 *  the caller, matching how `Driver.fromWdr` trusts its own parse today). */
export function fromYaml(text: string): OpenISDDriver {
  return OpenISDDriver.fromRecord(parse(text));
}

/** Serialise an `OpenISDDriver` to `openisd.yml`/`.owdr` text. See the file header —
 *  not yet byte-identical to the Python writer's canonical dialect. */
export function toYaml(driver: OpenISDDriver): string {
  return stringify(driver.toRecord(), { sortMapEntries: false });
}

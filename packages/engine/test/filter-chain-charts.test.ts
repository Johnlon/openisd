...
describe('EQ/filter chain charts — no filters means unity, exactly', () => {
  it('an empty chain is 0 dB, 0 rad and 0 ms at every frequency', () => {
    const sw = sweep(DRV, 'sealed', SEALED);
  ...
  it('a chain whose only filter is DISABLED is also unity — enabled is what counts', () => {
    ...
    const sw = sweep(DRV, 'sealed', { ...SEALED, filters });
...
describe('EQ/filter chain charts — the magnitude is the chain\'s own gain in dB', () => {
  ...
  it('cascaded filters multiply, i.e. their dB gains add', () => {
    ...
    const swA  = sweep(DRV, 'sealed', { ...SEALED, filters: [a] });
    const swB  = sweep(DRV, 'sealed', { ...SEALED, filters: [b] });
    const swAB = sweep(DRV, 'sealed', { ...SEALED, filters: [a, b] });
...
describe('EQ/filter chain charts — the chain is electrical, so driver and box do not enter it', () => {
  ...
  it('the same filters give the same chain response in a sealed and a vented box', () => {
    const s = sweep(DRV, 'sealed', { ...SEALED, filters });
    const v = sweep(DRV, 'vented', { ...VENTED, filters });
  ...
  it('force-flat does not appear in the chain — it is a real gain applied downstream', () => {
    const off = sweep(DRV, 'sealed', { ...SEALED, filters });
    const on  = sweep(DRV, 'sealed', { ...SEALED, filters, forceFlatResponse: true });
...
describe('EQ/filter chain charts — phase and group delay', () => {
  it('phase is unwrapped radians, and group delay is τg = −dφ/dω of that same phase', () => {
    const filters: Filter[] = [{ type: 'highpass', fc: 40, Q: Math.SQRT1_2, enabled: true }];
    const sw = sweep(DRV, 'sealed', { ...SEALED, filters });
    // Unwrapped: no step bigger than π between neighbours.
    for (let i = 1; i < sw.fltPhase.length; i++)
      assert.ok(Math.abs(sw.fltPhase[i] - sw.fltPhase[i - 1]) <= Math.PI,
        `phase jumped by more than π at ${sw.fs[i].toFixed(2)} Hz — not unwrapped`);
    // The exported τg applied to the exported phase must reproduce the exported delay:
    // the two charts cannot disagree about what group delay means.
    const expected = groupDelayMs(sw.fs, sw.fltPhase);
    for (let i = 0; i < expected.length; i++)
      assert.equal(sw.fltGd[i], expected[i], `fltGd[${i}] at ${sw.fs[i].toFixed(2)} Hz`);
  });
  ...
  it('unwrap is the shared definition — the chain uses it, so a wrapped input reproduces it', () => {
    const filters: Filter[] = [{ type: 'lowpass', fc: 80, Q: 1.2, enabled: true }];
    const sw = sweep(DRV, 'sealed', { ...SEALED, filters });
    // Re-wrapping then unwrapping the exported phase must return the exported phase.
    const wrapped = sw.fltPhase.map(p => Math.atan2(Math.sin(p), Math.cos(p)));
    const again = unwrap(wrapped);
    for (let i = 0; i < again.length; i++)
      assert.ok(Math.abs(again[i] - sw.fltPhase[i]) < 1e-9, `unwrap mismatch at ${sw.fs[i].toFixed(2)} Hz`);
  });
});

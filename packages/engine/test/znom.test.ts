 ...
 * hand — never a number read back from `nominalImpedance`.
 *
 ...
 * Both are asserted against that naive rival below, so the exactness cannot be silently lost.
 */

import { describe, it } from 'vitest';
import assert from 'node:assert/strict';
import { nominalImpedance, solveConsistencyGroup, type DriverRaw } from '@openisd/engine';
...
describe('Znom = 2·round_half_to_even(0.75·Re) — WinISD probe QO30', () => {
  it('reproduces all 16 swept Re values exactly', () => {
    for (const [label, Re, Znom] of PROBE) {
      assert.equal(nominalImpedance(Re), Znom, `${label}: Re=${Re} must give Znom=${Znom}`);
    }
  });

  it('rounds HALF TO EVEN — Re=6 refutes half-up, Re=10 refutes half-down', () => {
    assert.equal(nominalImpedance(6), 8, '0.75·6 = 4.5 → 4 (even), not 5 — half-up refuted');
    assert.equal(nominalImpedance(10), 16, '0.75·10 = 7.5 → 8 (even), not 7 — half-down refuted');
    assert.equal(nominalImpedance(2), 4, '0.75·2 = 1.5 → 2 (even) — confirms both directions');
  });

  it('evaluates the product EXACTLY, so the two near-ties beat the naive double route', () => {
    // The rival: round the double-rounded product, which is what `0.75 * Re` gives in JS.
    const naive = (Re: number): number => {
      const p = 0.75 * Re;
      const fl = Math.floor(p);
      const frac = p - fl;
      const n = frac > 0.5 ? fl + 1 : frac < 0.5 ? fl : (fl % 2 === 0 ? fl : fl + 1);
      return 2 * n;
    };

    assert.equal(0.75 * 7.333333333333333, 5.5, 'the double product IS the tie here');
    assert.equal(naive(7.333333333333333), 12, 'the naive route predicts 12…');
    assert.equal(nominalImpedance(7.333333333333333), 10, '…but WinISD wrote 10 (Z_tie_re7.333333)');

    assert.equal(0.75 * 3.3333333333333335, 2.5, 'the double product IS the tie here');
    assert.equal(naive(3.3333333333333335), 4, 'the naive route predicts 4…');
    assert.equal(nominalImpedance(3.3333333333333335), 6, '…but WinISD wrote 6 (Z_tie_re3.333333)');
  });

  it('Re = 0.6 gives a COMPUTED zero, not an absent value', () => {
    // WinISD saved Znom=0 marked C — distinct from the unset Znom=0/N of a blank driver.
    assert.equal(nominalImpedance(0.6), 0, 'Z_tie_re0.6');
    const r = solve({ ...BASE, Re: 0.6 });
    assert.equal(r.Z, 0, 'the solver must WRITE the zero, not leave Z absent');
  });
});
...
describe('Znom in the consistency solver', () => {
  it('derives Z from Re for every probe row', () => {
    for (const [label, Re, Znom] of PROBE) {
      ...
      assert.equal(r.Z, Znom, `${label}: solver must fill Z=${Znom} from Re=${Re}`);
    }
  });
  ...
  it('never corrects an entered Znom that contradicts Re', () => {
    ...
    const r = solve({ ...BASE, Re: 6, Z: 4 });
    assert.equal(r.Z, 4, 'an entered Z is pinned — the rule never overwrites it');
  });
  ...
  it('follows Re, not the damping factors', () => {
    ...
    assert.equal(r.Z, 12, '2·round(0.75·8) = 12, not the 40 that Re=27 would give');
  });
  ...
  it('accepts a CALCULATED Re as its input', () => {
    ...
    assert.equal(r.Z, 8, 'and Znom follows the derived Re');
  });
  ...
  it('leaves Z absent when Re is unknown', () => {
    ...
    assert.equal(r.Z, undefined, 'no Re → no Znom (slot 0 stays N)');
  });
});

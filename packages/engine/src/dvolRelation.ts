 ...
 * directions, reverse-engineered and pinned in `docs/design/WDR_SCHEMA.md` §3.10.1 /
 ...
 * `MagDepth`. `S = Dd² + Dd·Vcd + Vcd²` is the frustum's cross-section shorthand WDR_SCHEMA
 ...
 *
 * The four expressions are algebraically equivalent inverses of one relation (WDR_SCHEMA:
 ...
 * MagDepth` (the cone's height must be positive) — WDR_SCHEMA's binary trace guards the target
...
/** `Magnet` from `DVol` and the other three. The inverse takes a square root — WDR_SCHEMA notes

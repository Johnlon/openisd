...
export function defaultFilter(type: FilterType): Filter {
  ...
  // No default arm: `type` is `FilterType`, a closed 6-member union, and every call site is
  // either a literal from that union or a value parsed through `filterJsonSchema`'s matching
  // `z.enum(...)` (openisdSchema.ts) — nothing untyped can reach this switch. Leaving the
  // default off (rather than a `never`-typed throw) means an unhandled new variant fails to
  // COMPILE ("not all code paths return a value") instead of merely failing to be dead code
  // at runtime.
}

...
describe('structural DQ on ReadonlyField<T>', () => {
  it('exposes dq on ReadonlyField<T>, an empty list for a valid field', () => {
    const p = project();
    const cell = p.box.vented.volume_m3;
    expect(Array.isArray(cell.dq)).toBe(true);

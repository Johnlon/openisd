 ...
 *
 * `#cursorF`, `#pinnedF`, `#cursorLocked`, `#dragRange` are a fourth exception (QO168, John
 * 2026-09-21): the graph crosshair/drag-selection. S10/QO130 made the cursor PROJECT-scoped (two
 * open projects must not share one), but it writes on every mousemove/drag — going through
 * `#slot()` like an ordinary field would clone the record into `#edited` and re-run the full
 * `#resolve()` solve cascade on every pointer move, for a value the solver never reads. John
 * ruled these four OUT of the saved record entirely: never `OpenISDProjectJson`, never
 * `#saved`/`#edited`/`#whatif`, never `.owpr`/a share link. They reset to defaults on every
 * (re)wrap, same as `#issues` holds no information a fresh resolve wouldn't reproduce.
 */

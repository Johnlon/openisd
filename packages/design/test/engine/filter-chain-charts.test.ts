...
describe('EQ/filter chain charts — no filters means unity, exactly', () => {
  it('an empty chain is 0 dB, 0 rad and 0 ms at every frequency', () => {
    const sw = engine.sweep(driverParams(DRV), LE_H, 'sealed', SEALED).values!;
...
describe('EQ/filter chain charts — the magnitude is the chain\'s own gain in dB', () => {
  ...
  it('cascaded filters multiply, i.e. their dB gains add', () => {
    ...
    const swA  = engine.sweep(driverParams(DRV), LE_H, 'sealed', { ...SEALED, filters: [a] }).values!;
    const swB  = engine.sweep(driverParams(DRV), LE_H, 'sealed', { ...SEALED, filters: [b] }).values!;
    const swAB = engine.sweep(driverParams(DRV), LE_H, 'sealed', { ...SEALED, filters: [a, b] }).values!;
...
describe('EQ/filter chain charts — the chain is electrical, so driver and box do not enter it', () => {
  ...
  it('the same filters give the same chain response in a sealed and a vented box', () => {
    ...
    const v = engine.sweep(driverParams(DRV), LE_H, 'vented', { ...VENTED, filters }).values!;
  ...
  it('a different driver leaves the chain response untouched', () => {
    ...
    const a = engine.sweep(driverParams(DRV), LE_H,   'sealed', { ...SEALED, filters }).values!;
    const b = engine.sweep(driverParams(other), LE_H, 'sealed', { ...SEALED, filters }).values!;
  ...
  it('force-flat does not appear in the chain — it is a real gain applied downstream', () => {
    const off = engine.sweep(driverParams(DRV), LE_H, 'sealed', { ...SEALED, filters }).values!;
    const on  = engine.sweep(driverParams(DRV), LE_H, 'sealed', { ...SEALED, filters, forceFlatResponse: true }).values!;
...
describe('EQ/filter chain charts — Linkwitz transform, low-shelf, high-shelf', () => {
  it('a Linkwitz transform\'s gain at a very low frequency matches the closed-form biquad ratio', () => {
    // H(s) = (s²+(w0/Q0)s+w0²) / (s²+(wp/Qp)s+wp²), independently evaluated here (not by
    // importing filters.ts — the engine door forbids that) and compared to the sweep's own
    ...
    const expectedDb = 20 * Math.log10(numMag / denMag);
    const i = nearest(sw.fs, TEST_F);
    assert.ok(Math.abs(sw.fltMag[i] - expectedDb) < 0.05,
      `expected ≈ ${expectedDb.toFixed(3)} dB at ${sw.fs[i].toFixed(3)} Hz, got ${sw.fltMag[i].toFixed(3)} dB`);
  });

  it('a low-shelf boost is at full gain well below fc (explicit Q)', () => {

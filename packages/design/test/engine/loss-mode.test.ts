...
describe('Sealed-Box Resonance Loss Models', () => {
  describe('WinisdLossy — bit-exact to WinISD readout', () => {
    ...
    for (const [Ql, fr] of oracle) {
      it(`Vb=6L Ql=${Ql} → ${fr} Hz`, () => {
        closeTo(winisdFsc(p(0.006, Ql)), fr, 4, `Fsc at Ql=${Ql}`);
      });
    }
    // Other volumes at Ql=10 (same driver) — from the oracle grid.
    it('tracks volume at Ql=10 (10L, 20L, 40L)', () => {
      closeTo(winisdFsc(p(0.010, 10)), 55.14994, 3, 'Fsc at 10 L');
      closeTo(winisdFsc(p(0.020, 10)), 48.40568, 3, 'Fsc at 20 L');
      closeTo(winisdFsc(p(0.040, 10)), 44.50218, 3, 'Fsc at 40 L');
    });
  });
  ...
  describe('WinisdLossy converges to lossless as leakage vanishes', () => {
    it('Ql→∞ equals fs·√(1+Vas/Vb)', () => {
      ...
      closeTo(winisdFsc(p(0.006, 1e9)), lossless, 6, 'Fsc at Ql=1e9');
    });
    it('resonance rises monotonically as Ql falls', () => {
      const f = (Ql: number) => winisdFsc(p(0.006, Ql));
  ...
  // Direct tests of the pole solver itself — where the Qtc half of the readout and the two
  // internal branches (complex pole pair vs the overdamped real-root fallback) are exercised
  // head-on.
  describe('the WinISD-lossy pole solver, direct', () => {
    it('short-circuits to the lossless closed form when there is no leakage', () => {
      ...
      for (const Ql of [1e6, 1e9, 0, -5]) {
        const r = winisdLossy(p(0.006, Ql));
    ...
    it('treats a non-positive or effectively-infinite Qa as no absorption loss', () => {
      ...
      const base = winisdLossy(p(0.006, 10, 1e9));
      for (const Qa of [0, -1, 1e6, 1e12]) {
        assert.deepEqual(winisdLossy(p(0.006, 10, Qa)), base,
          `Qa=${Qa} must read as no absorption loss`);
      }
    });

    it('adding absorption loss lowers Q without moving resonance much', () => {
      const light = winisdLossy(p(0.006, 10, 10000));
      const heavy = winisdLossy(p(0.006, 10, 20));

...
/**
 * `OpenISDDeviceJson` IS the openisd.json file, declared as a type. This is the only check that
 ...
 *
 * The two lists below come from the pydantic model that WRITES the record — `OpenIsdJsonFile` in
 ...
 */
describe('OpenISDDeviceJson matches the openisd.json record', () => {
  // THE AUTHORITY IS THE PYDANTIC MODEL. `OpenIsdJsonFile` in winisd_tools
  // `scrapers/scrapers/lib/model_openisd.py` writes every openisd.json and is `extra="forbid"`;
  ...
  it('declares every key an openisd.json record carries', () => {
    const declared = new Set(members().keys());
    const missing = [...ALWAYS, ...SOMETIMES].filter(k => !declared.has(k));
    expect(missing, `openisd.json keys the type does not model: ${missing.join(', ')}`).toEqual([]);
  });

...
specs:
  woofer:
    Re_ohm:
      ...
      readings:
        manufacturer_datasheet: {actual_reading: '5.6 Ohm', read_value: 5.6, read_precision: 0.05}
`;
...
describe('openisd-bridge.js — artifact shape', () => {
  ...
  it('tree-shakes TextEncoder/TextDecoder out entirely (they are absent from mini-racer)', () => {
    // The bridge DOES reach winisdBytes.ts now — `winisdTextToBytes` produces the .wdr bytes
    // it returns — but that function encodes through `utf8.ts`, which derives UTF-8
    // arithmetically for exactly this reason. `TextDecoder` is left behind with
    // `winisdBytesToText`, the file-READ direction, which nothing on this path calls. If a
    // future change makes either survive bundling, that is a real change to what the bridge
    // touches and must be reported, not silently polyfilled.
...
describe('openisd-bridge.js — behavioural (node:vm, mini-racer-shaped sandbox)', () => {
  ...
  it('converts a real driver.json record to openisd.json + .wdr, base64 file BYTES, CRLF-preserved', () => {
    assert.equal(existsSync(REAL_DRIVER_JSON), true, `fixture missing: ${REAL_DRIVER_JSON}`);
    const jsonText = readFileSync(REAL_DRIVER_JSON, 'utf8');
    ...
    // openisd.json is UTF-8 JSON and parses as itself.
    JSON.parse(openisdBytes.toString('utf8'));
  });

  it('collapses a comment newline to the single 0xA4 byte the .wdr format specifies', () => {
    // The sentinel U+F8A4 is openisd's IN-MEMORY stand-in for that byte. It must never reach
    // the caller: its UTF-8 is EF A2 A4, and WinISD splits a value on a raw 0xA4, so a caller
    // encoding the text itself writes three bytes where the format has one.
    const ctx = createMiniRacerLikeContext();
    vm.runInContext(bundleSource, ctx);

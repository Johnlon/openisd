/**
 * @openisd/design/winisd — what a `.wdr` STATES must survive import, and what it does not state must
 * not be invented.
 *
...
describe('a .wdr key the file does carry survives import unchanged', () => {
  ...
  it('a .wdr stating SPL=90 reports 90, marked ENTERED', () => {
    const driver = driverOf(SEALED_SMALL);
    const cell = driver.spec[driver.section].SPL_dB.get();
    assert.equal(cell.state, 'entered',

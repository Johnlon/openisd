...
export interface AppContext {
  ...
  now(): Date;
  /** Whoever is using this machine, or null when nobody has said — the default for a new
   *  driver's `providedBy`, a new project's `creator`, a new PR's `providedBy`. The domain has
   *  no OS-username source of its own; a host that has one (the app's own Username setting,
   *  an OS call) supplies it by overriding this method. */
...
export const realAppContext: AppContext = Object.freeze({
  ...
  now: (): Date => new Date(),
  platformUser: (): string | null => null,
});

/** WinISD's own date-stamp text, `YYYYMMDD` — the format `created`/`modified`/`added` are
 *  stored in, matching a `.wpr`'s `CreateDate`/`ModifyDate`. */

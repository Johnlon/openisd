...
const FIELDS: FieldSpec[] = [
  ...
  { id: 'Mcost', label: 'Mcost', pane: 'Driver: Advanced', kind: 'number', unit: 'kg/s', precision: 5, min: 0, max: 1000, provenance: 'calculated', modeled: false, appliesTo: 'all', formula: 'Mcost = Rme·(1 + Xmax/min(Hc, Hg))', dependsOn: ['Rme', 'Xmax', 'Hc', 'Hg'], description: 'Motor figure of merit (WinISD 5 dp). Blank when Hc or Hg is absent or zero, since they are the divisor — which is why it reads 0 across most of the library. Reduces to [Rme] exactly at Xmax = 0. Recovered from live WinISD probes to 1.5e-15 (ledger QO32).' },
  { id: 'Gloss', label: 'Gloss', pane: 'Driver: Advanced', kind: 'number', unit: '%', precision: 4, min: 0, max: 100, provenance: 'calculated', modeled: false, appliesTo: 'all', formula: 'Gloss = g/((2π·Fs)²·Xmax), g = 9.80665', dependsOn: ['Fs', 'Xmax'], description: 'Cone sag under gravity when the driver is mounted horizontally, as a fraction of Xmax (WinISD 4 dp). The record and the .wdr carry the FRACTION; this pane shows the percent, and the ×100 lives only in the input\'s scale. Recovered from live WinISD probes to 3.6e-15 (ledger QO32).' },

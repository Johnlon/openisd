...
import { upsertMyDriver, removeMyDriver, driverId } from '../utils/myDrivers.js';
 ...
 */
export async function selectDriver(f: LibraryEntry): Promise<SelectionResult> {
  let m: DriverModel;
  if (f.myDriverData) {
...
export function editMyDriver(d: DriverRaw): void {
 ...
 */
export function openNewDriver(): void {
...
export function editorSeed(): { json: DriverJSON; subject: EditorSubject['kind'] } {
  return {
 ...
 */
export function acceptDriverEdit(json: DriverJSON): void {
  if (subject.kind === 'myDriver') {
    ...
    if (subject.openedAs && subject.openedAs !== driverId(raw)) removeMyDriver(subject.openedAs);
    upsertMyDriver(raw);
  } else {
    setDriverFromSerialized(json);
  }
...
export function cancelDriverEdit(): void {
  closeEditor();
}

...
const FIELDS: FieldSpec[] = [
  ...
  { id: 'AlfaVC', label: 'AlfaVC', pane: 'Driver: Advanced', kind: 'number', unit: '1000/K', precision: 4, min: 0, max: 100, provenance: 'entered', modeled: true, appliesTo: 'all', description: 'Voice-coil resistance temperature coefficient (copper 3.9000; labelled "Voice coil resistance TC" on the Driver pane). OpenISD now uses it — scales hot Re with the temp rise (Re_hot=Re·(1+alfaVC·ΔT), circuit.ts). WinISD parity (WINISD.md §12c). Stored SI (/K) = this value ÷ 1000.' },
  ...
  { id: 'vcTempRise', label: 'Voice coil temp rise', pane: 'Driver', kind: 'number', unit: 'K', precision: 2, min: 0, max: 500, provenance: 'entered', modeled: true, appliesTo: 'all', description: 'Voice-coil ΔT above ambient (0.00 K, 2 dp). Power compression: OpenISD now models Re_hot=Re·(1+alfaVC·ΔT) in circuit.ts — impedance floor rises, SPL drops. WinISD parity (WINISD.md §12c). 0 = no compression.' },
  { id: 'driverAddedMass', label: 'Added mass to cone', pane: 'Driver', kind: 'number', unit: 'g', precision: 5, min: 0, max: 5000, provenance: 'entered', modeled: true, appliesTo: 'all', description: 'Driver-side added cone mass (native grams, 5 dp). OpenISD now models it (engine withAddedMass): Mms += Madd → Fs lowers, Qts rises. Verified vs WinISD (70→25 Hz for +100 g, WINISD.md §12c). 0 = no-op.' },

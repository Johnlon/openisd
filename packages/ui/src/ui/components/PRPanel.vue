...
import { state, driver, enterPrField, prFieldState, prTargetUnreachable } from '../../logic/store.js';
import { prMassForFp, ventedAlignment } from '@openisd/engine';
...
// The system tuning, kept solved by the store's PR group. Read it from there rather than
// recomputing: when the user ENTERS a target tuning it is the authoritative value and the
// added mass is what gets solved, so a local prTuning() call would fight the entered value.
const fp = computed<number>({
  get: () => P.value.prFp,
  set: (v: number) => enterPrField('prFp', v),
});
const madd = computed<number>({
  get: () => P.value.prMadd,
  set: (v: number) => enterPrField('prMadd', v),
});
const fpState = computed(() => prFieldState('prFp'));
const maddState = computed(() => prFieldState('prMadd'));
const targetUnreachable = computed(() => prTargetUnreachable());
...
<template>
  ...
  <!-- PR tuning — always visible; not intrinsic to the device. The PR's own Sd/Cms/Mms/Rms
       are specs of the part you bought and live in the collapsed editor above; what you
       actually design with is how many radiators, and the tuning ↔ added-mass pair. -->
  <div class="subsect">PR tuning</div>
  <div class="row" title="Number of passive radiators. More radiators raise output without changing the tuning frequency Fp, so this is a design choice, not a property of the PR. WinISD: 'Number of radiators'.">
    <label>PR count</label>
    <NumInput v-model="state.P.prNum" :scale="1" :precision="2" step="1" :min="1" />
    <span class="u"></span>
  </div>
  <!-- Tuning and added mass are one relation with two members: enter the target tuning and
       the mass is solved, or enter the mass and the tuning is. prMassForFp() is the exact
       inverse of prTuning() and both already existed — only the mass direction was wired. -->
  <div class="row" title="Target system tuning in this box (WinISD: Fp). Enter it and the required added mass is solved.">
    <label>Tuning Fp</label>
    <NumInput v-if="fpState === 'E'" v-model="fp" :scale="1" :precision="2" />
    <span v-else class="pr-roval">{{ fpState === 'N' ? '—' : fp.toFixed(2) }}</span>
    <span class="u">Hz</span>
  </div>
  <div class="row" title="Extra mass physically bolted to the PR cone (e.g. steel washer, lead shot). Shifts the tuning frequency down without changing the PR's other parameters. WinISD: 'Added mass'.">
    ...
    <NumInput v-if="maddState === 'E'" v-model="madd" :scale="1000" :precision="4" />
    <span v-else class="pr-roval">{{ maddState === 'N' ? '—' : (P.prMadd * 1000).toFixed(2) }}</span>
    <span class="u">g</span>
  </div>
  <div v-if="targetUnreachable" class="pr-warn">
    This PR cannot tune that high in this box — mass can only be added, never removed. Use a
    lighter radiator, a smaller box, or a lower target.
  </div>

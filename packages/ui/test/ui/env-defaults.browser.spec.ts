...
test('BUG 6: temperature/humidity/pressure show the project\'s own stored values, not blank', async ({ page }) => {
  // COMPLETE_DRIVER_PROJECT_OWPR stores its own env (generateSample.ts): 293.15 K, RH 50 %,
  // 101325 Pa. Temp/pressure happen to match the app defaults too; humidity does not — the
  // project's stored value wins over the app default because it was entered, not deleted
  // (John, 2026-09-21: app defaults flow through to the project's Advanced tab only when the
  // project's own field has been deleted, never as an override of an explicit stored value).
...
test('BUG: a project built from the wizard shows the air constants, and they recompute on input', async ({ page }) => {
  ...
  await page.locator('.use-btn').click();                       // choosing the driver lands step 2
  ...
  await modal.locator('button', { hasText: 'Next' }).click();   // step 5: metadata
  await modal.locator('input[type="text"]').fill('Env project');
...
test('the app-level Options Environment tab always shows the app default 30 % humidity, unaffected by any loaded project', async ({ page }) => {
  // COMPLETE_DRIVER_PROJECT_OWPR stores its own humidity (50 %, see BUG 6 above) on the
  // PROJECT's Advanced tab. The Options dialog's Environment section is a separate, app-level

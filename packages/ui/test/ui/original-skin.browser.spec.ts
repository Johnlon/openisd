...
test('Signal tab: Driver input voltage is editable and drives System input power (W↔V, P=V²/Re)', async ({ page }) => {
  await page.locator('.project-nav li', { hasText: 'Signal' }).click();
  const pinField = page.locator('.field', { hasText: 'System input power' }).locator('input');
  const before = await pinField.inputValue();
  ...
  await vInput.blur();
  await expect(pinField).not.toHaveValue(before);
});
...
test('New Project walks driver → num/placement → box type → alignment → name, and the name shows in the titlebar', async ({ page }) => {
  await page.locator('.tb-btn[title*="New project"]').click();
  const modal = page.locator('.overlay.open');
  // FIX_WIZARD_SEALED + Q1 order: driver (embedded library) → num/placement → box type → alignment → name.
  ...
  await modal.locator('button', { hasText: 'Next' }).click();  // Use landed step 2; → step 3: box type
  ...
  await modal.locator('input[type="text"]').fill('My Sub Build');
...
test('New Project starts fresh — it discards the previous design (filters, params)', async ({ page }) => {
  ...
  await modal.locator('button', { hasText: 'Next' }).click();   // step 4: sealed alignment
  await modal.locator('button', { hasText: 'Next' }).click();   // step 5: name
  await modal.locator('input[type="text"]').fill('Skin project');
  await modal.locator('button', { hasText: 'Create' }).click();

  // The wizard creates the project on Create; then the fresh project is focused.
  await expect(page.locator('.original-root')).toBeVisible();

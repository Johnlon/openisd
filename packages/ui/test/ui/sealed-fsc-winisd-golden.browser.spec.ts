...
test('sealed box WinISD golden: Fs=40 Vas=7.65L Qes=0.45 Qms=2.94 Re=6.6 Rg=0.1 Vb=6L Ql=10 Qa=100 → Fsc=63.1762Hz Qtc=0.5995', async ({ page }) => {
  ...
  await setNumField(page, 'Qms', 2.940, tune);
  await setNumField(page, 'Vas', 7.65, tune);   // scale=1000: litres in, m³ stored
  await setNumField(page, 'Re', 6.6, tune);
  ...
  await setNumField(page, 'Leakage Ql', 10);
  await setNumField(page, 'Absorption Qa', 100);

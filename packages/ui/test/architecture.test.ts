...
const COMPOSABLES_DIR = join(UI_SRC, 'composables');
const SHELLS_DIR = join(UI_SRC, 'shells');
...
describe('UI Architecture Boundaries', () => {
  it('composables have no UI/view component dependency (.vue)', () => {
    ...
    for (const f of files) {
      const text = readFileSync(join(COMPOSABLES_DIR, f), 'utf8');
      assert.ok(
        !forbiddenVueImport.test(text),
        `Composable packages/ui/src/composables/${f} imports a .vue view component — logic must stay decoupled from presentation`
      );
    }
  });

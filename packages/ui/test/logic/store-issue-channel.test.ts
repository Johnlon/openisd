...
describe('the store unions every hardening layer into one issue list', () => {
  it('a fully specified design is clean — no layer reports a false positive', async () => {
    ...
    requireFocusedProject().driver.specs.Fs_hz.set(37);
    requireFocusedProject().driver.specs.Qts.set(0.378);
    requireFocusedProject().driver.specs.Qes.set(0.40);
    requireFocusedProject().driver.specs.Qms.set(7.0);
    requireFocusedProject().driver.specs.Vas_m3.set(0.0300);
    requireFocusedProject().driver.specs.Sd_m2.set(0.0133);
    requireFocusedProject().driver.specs.Re_ohm.set(5.6);
    requireFocusedProject().driver.specs.Le_H.set(0.70e-3);
    requireFocusedProject().driver.specs.Xmax_m.set(0.0050);
    requireFocusedProject().driver.specs.Pe_W.set(60);
    requireFocusedProject().driver.specs.Znom_ohm.set(8);
  ...
  it('carries one sweep circuit failure per missing driver value, not one combined message', async () => {
    ...
    const driver = requireFocusedProject().driver.specs;
    driver.Fs_hz.set(37);
...
it('an unsized vent port surfaces a tuning_goal_hz/length_m error through allIssues, '
  ...
  it('a vent sweep error reads the identical sentence the cell itself carries as DQ (S2-11)', async () => {
    ...
    const swept = allIssues.value.find(e => e.field === 'tuning_goal_hz' && e.level === 'error');

...
test('cloning a library driver does not link its favourite star to the original', async ({ page }) => {
  await seed(page, []);
  await openPicker(page);

  const original = page.locator(POOL_ROWS).first();
  await expect(original, 'no library rows rendered — this spec would prove nothing').toBeVisible();
  const originalName = (await original.locator('b').textContent())?.trim();

  await original.locator('.fav-btn').click();
  await expect(original.locator('.fav-btn'), 'starring the original failed').toHaveClass(/on/);

  await original.click();
  await page.locator('.wb-modal .clone-btn').click();
  await page.locator('.wb-modal .cancel-btn').click();

  const clone = page.locator(MY_ROWS).filter({ hasText: 'Copy of' });
  await expect(clone, 'the clone did not appear in My Drivers').toHaveCount(1);
  await expect(clone.locator('.fav-btn'),
    "the clone inherited the original driver's favourite star — they must be independent entries")
    .not.toHaveClass(/on/);

  // Star the clone too, then check each row's star answers only for itself — proves the two
  // rows do not share one favourites key (e.g. a duplicate/aliased uuid).
  await clone.locator('.fav-btn').click();
  await expect(clone.locator('.fav-btn'), 'starring the clone failed').toHaveClass(/on/);
  const originalAfter = page.locator(POOL_ROWS).filter({ hasText: originalName! }).first();
  await expect(originalAfter.locator('.fav-btn'), 'starring the clone un-starred the original').toHaveClass(/on/);

  await originalAfter.locator('.fav-btn').click();
  await expect(originalAfter.locator('.fav-btn'), 'un-starring the original failed').not.toHaveClass(/on/);
  await expect(clone.locator('.fav-btn'), 'un-starring the original also un-starred the clone').toHaveClass(/on/);
});
...
test('editing a library driver\'s overview and saving to My Drivers does not link its favourite star to the original', async ({ page }) => {
  await seed(page, []);
  await openPicker(page);
  ...
  const edited = page.locator(MY_ROWS).filter({ hasText: 'Edited Copy' });
  await expect(edited, 'the edited driver did not land in My Drivers').toHaveCount(1);
  await expect(edited.locator('.fav-btn'),
    "the edited My Drivers entry inherited the library original's favourite star — they must be independent entries")
    .not.toHaveClass(/on/);

  // Star the new entry too, then check each row's star answers only for itself — proves the two
  // do not share one favourites key (e.g. the save kept the original's domain uuid).
  await edited.locator('.fav-btn').click();
  await expect(edited.locator('.fav-btn'), 'starring the edited entry failed').toHaveClass(/on/);
  const originalAfter = page.locator(POOL_ROWS).filter({ hasText: originalName! }).first();
  await expect(originalAfter.locator('.fav-btn'), 'starring the edited entry un-starred the original').toHaveClass(/on/);

  await originalAfter.locator('.fav-btn').click();
  await expect(originalAfter.locator('.fav-btn'), 'un-starring the original failed').not.toHaveClass(/on/);
  await expect(edited.locator('.fav-btn'), 'un-starring the original also un-starred the edited entry').toHaveClass(/on/);
});

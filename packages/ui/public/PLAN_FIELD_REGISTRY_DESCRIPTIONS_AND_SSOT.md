...
| `vent_EndCorrection` | - | Vents | `REWRITE` | `End Correction Factor: Coefficient accounting for air mass dragging outside port ends (0.613 free, 0.732 flanged baffle, 0.849 double flanged), adding effective acoustic length.` |
...
| `vent_PortResonance_hz` | - | Vents | `REWRITE` | `First Vent Pipe Resonance: Half-wave standing wave resonance inside the port tube (f = c / 2L), causing pipe-mode resonance peaks in passband output.` |

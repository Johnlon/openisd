...
import { winisdBytesToText, winisdTextToBytes, WINISD_NEWLINE_SENTINEL, WinisdEncoding } from '../src/winisdBytes.js';
...
describe('the 0xA4 newline sentinel in a .wdr string field', () => {
  it("decodes WinISD's own unicode sample — Euro, Kanji and two embedded newlines", () => {
    const { text, encoding } = winisdBytesToText(new Uint8Array(readFileSync(UNICODE_WDR)));
    const comment = /^Comment=(.*)$/m.exec(text)?.[1];

    assert.equal(encoding, WinisdEncoding.Utf8, "WinISD's own sample is valid UTF-8");
    assert.equal(comment, `Euro €${WINISD_NEWLINE_SENTINEL}Kanji 漢字${WINISD_NEWLINE_SENTINEL}`,
  ...
  it('a 0xA4 INSIDE a multi-byte sequence is data, not a sentinel', () => {
    ...
    const { text, encoding } = winisdBytesToText(bytes);
    assert.equal(encoding, WinisdEncoding.Utf8);
    assert.equal(text, `C=¤${WINISD_NEWLINE_SENTINEL}x`);
  });

  it('bytes -> text -> bytes is the identity on a WinISD-written file', () => {
    const original = new Uint8Array(readFileSync(UNICODE_WDR));
    assert.deepEqual(winisdTextToBytes(winisdBytesToText(original).text), original);
  });

  it('a multi-line Comment survives a full driver round trip and stays on one line', () => {
    const source = winisdBytesToText(new Uint8Array(readFileSync(UNICODE_WDR))).text;
    ...
    assert.equal(commentLines[0],
      `Comment=Euro €${WINISD_NEWLINE_SENTINEL}Kanji 漢字${WINISD_NEWLINE_SENTINEL}`);
  });
...
describe('CP1252 fallback for a .wdr that is not valid UTF-8 (QO62)', () => {
  ...
  it('the CP1252 fallback still recognises 0xA4 as the newline sentinel, whole-file, not per-line', () => {
    ...
    assert.equal(encoding, WinisdEncoding.Cp1252);
    assert.equal(text, `A${WINISD_NEWLINE_SENTINEL}•${WINISD_NEWLINE_SENTINEL}B`);
  });

  it('the real Selenium SW108 fixture is not valid UTF-8 and reads back with (R) (0xAE) intact', () => {
    const { text, encoding } = winisdBytesToText(new Uint8Array(readFileSync(SELENIUM_SW108_WDR)));

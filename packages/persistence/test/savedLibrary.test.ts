...
describe('both saved libraries return live domain objects', () => {
  it('My Drivers reads back an OpenISDDriver carrying every stored field', () => {
    ...
    assert.equal(back[0].driver.model.get().value, 'RS225');
    assert.equal(back[0].driver.brand.get().value, 'Dayton');
  });
  ...
  it('My Passive Radiators reads back a radiator, not a five-number row', () => {
    ...
    assert.equal(back[0].passiveRadiator.model.get().value, 'DSA175-PR');
    assert.equal(back[0].passiveRadiator.brand.get().value, 'Dayton');
  });

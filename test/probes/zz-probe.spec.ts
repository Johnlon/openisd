...
test('probe: Frc fill round-trip', async ({ page }) => {
  ...
  const refs = await page.evaluate(async (modPath) => {
    ...
    const viaFocus = S.focusedProject()?.box.bandpass6.chambers.rear.tuning_hz.value ?? 'NULL-PROJ';
    const viaOpen = S.openProjects()[0]?.box.bandpass6.chambers.rear.tuning_hz.value ?? 'NO-OPEN';

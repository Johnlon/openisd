...
| 5   | `SimulatorOptions` (`VCInd`/`FlatResponse`/`TLPorts`) actual behaviour in WinISD — still unobserved (all-zero corpus). OpenISD now WRITES all three from real design state and documents the semantics it implements as assumptions (`WINISD.md` §12d). Also open: WinISD's other two Advanced toggles (`Rg is at driver side`, `SPL graph is Xmax limited`) have no key in this format at all — OpenISD deliberately omits them rather than inventing keys. | Low — never toggled in this corpus                                                                                     |

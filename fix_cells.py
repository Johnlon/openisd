...
def replace_calls(content):
    for field, mapped in field_map.items():
        # Because we previously ran `content = re.sub(rf'\b([a-zA-Z0-9_]+)\.{field}Cell\(\)', rf'\1.spec[\1.section].{mapped}', content)`
        # The code now has `driver.spec[driver.section].Fs_hz` where it USED to have `driver.FsCell()`.
        # Wait, if we just do `.Fs_hz` and the user expects an object with `.value` and `.state`, we need `.Fs_hz.get()`.
        # So we can replace `.Fs_hz.` with `.Fs_hz.get().`?
        # That's too risky.
        pass
    return content

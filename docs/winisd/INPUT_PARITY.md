# WinISD ↔ Resonate — input & feature parity

Evidence-based comparison built by reading the WinISD 0.7.0.950 screenshots in
this folder against Resonate's actual UI (`packages/ui/src/components/DriverDefineModal.vue`
...
Legend: ✅ Resonate has an input/feature · ❌ Resonate lacks it · ⚠️ partial / different.
...
| Figure-of-merit: **SPLmaxLF, SPLmax, Rme, gamma, Mpow, Mcost, Gloss** | ❌ (WinISD shows these derived values; Resonate shows none) |
...
| **Dvol** (driver displacement volume)                  | ❌ (Resonate has a Weight field instead) |
...
Volume, Fh, and an Advanced→ (losses). Resonate has Vb + Ql/Qa/Qp. ✅

## Passive Radiator pane (`view_3_passive_radiator.png`) — parity

Vas, Fs, Qms, Sd, Xmax, Num. of PRs, Added mass, Fs-with-added-mass. Resonate's PR panel
...
| WinISD field / toggle                                                                   | Resonate                                              |
| --------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| **Temperature, Relative humidity, Air pressure** → derived Sound velocity + Air density | ❌ none — Resonate hardcodes c/roo                    |
...
| **SPL graph is Xmax limited**                                                           | ❌ (Resonate has a separate Max-SPL chart)            |

## Project pane (`view_7_advanced.png`) — Creator/Created/Modified/Description metadata. Resonate saves projects but has no per-project author/date/description fields. ⚠️
...
Resonate: SPL, Excursion, Port velocity, Group delay, Impedance mag, Impedance phase, Transfer
phase, Max-SPL, Max-power.

Resonate lacks: **Amplifier apparent load power (VA)** (BACKLOG P2), port **Gain** curves (has
...
Resonate hardcodes `C = 345.0`, `RHO = 1.184` (constants.ts) — labelled "20 °C" but actually
~24 °C. Correcting to **c ≈ 343.7, ρ ≈ 1.201** fixes the mislabel, matches textbook 20 °C, _and_
brings Resonate in line with WinISD. Cost: every result shifts ~0.4 %, so the golden-master
...
1. **Environment model** — WinISD derives c/ρ from temperature/humidity/pressure (per-project
   and app-default). Resonate has a single hardcoded, mislabelled, slightly-wrong constant.
2. **Off-axis + configurable listening distance** — Resonate is fixed 1 m on-axis.

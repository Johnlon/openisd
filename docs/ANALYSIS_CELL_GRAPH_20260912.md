...
| --- | --- | --- | --- |
| PR tuning ↔ cone mass | `tuning_goal_hz` (Fp) ↔ `addedMass_kg` | `solvePrConsistencyGroup` | negative mass → DQ on Fp/Madd/systemTuning/Fpr-mass |
...
| Vent tuning ↔ port length | `tuning_goal_hz` (Fb) ↔ vent `length_m` | `solveVentConsistencyGroup` | negative length → DQ on Fb + length |

...
(the layering this plan must respect) · [docs/design/DRIVER_ADT_DESIGN.md](http://localhost:8000/winisd/openisd/docs/design/DRIVER_ADT_DESIGN.md#L1)
...
[WINISD_SCHEMA.md §4](http://localhost:8000/winisd/openisd/docs/design/WINISD_SCHEMA.md#L248-L271) already
...
([WINISD_SCHEMA.md:232-234](http://localhost:8000/winisd/openisd/docs/design/WINISD_SCHEMA.md#L232-L234)). The
...
| 5     | Qts, Qms, Qes        | `Qts = Qms·Qes/(Qms+Qes)`            | Binary string; empirically re-verified [WINISD_SCHEMA §5.1](http://localhost:8000/winisd/openisd/docs/design/WINISD_SCHEMA.md#L275-L302), reproduce via `drivers/mysamples/inconsistency-test-saved-q.wdr`                                                                                                                                                                                             | DQ-tolerance check ([`:454-455`](http://localhost:8000/winisd/winisd_tools/scrapers/scrapers/lib/model_wdr.py#L454-L455))                                                                                                                                                                                          | guarded ([`:369-371`](http://localhost:8000/winisd/openisd/packages/winisd/src/driver.ts#L369-L371))                              | guarded ([`:73-75`](http://localhost:8000/winisd/openisd/packages/engine/src/driver.ts#L73-L75))                                                                                                                                                                                                                    |
...
| 19    | Xmax, Hc, Hg         | `Xmax = abs(Hc−Hg)/2`                | **Observed firing in WinISD** — probe `A_hchg_only`/`C_all_four`, [`runs/xmax_route.jsonl`](http://localhost:8000/winisd/winisd_research/runs/xmax_route.jsonl); matches the help text, [thielesmall.html](http://localhost:8000/winisd/docs/winisd_helpfiles/help/thielesmall.html#L180-L182): _"usually calculated as abs(Hc-Hg)/2 … WinISD does not multiply with any of these factors"_ | not derived (stores `missing:Xmax`)                                                                                                                                                                                                                                                                                | live — `Driver#derive()` calls `solveConsistencyGroup` directly (§3)                                                              | live: [`:151-152`](http://localhost:8000/winisd/openisd/packages/engine/src/driver.ts#L151-L152), evaluated ahead of row 20                                                                                                                                                                                         |
| 20    | Vd, Sd, Xmax         | `Vd = Sd·Xmax`, all three directions | Binary string; `Xmax = Vd/Sd` observed — probe `B_vdsd_only`                                                                                                                                                                                                                                                                                                                                  | **3 sites**, m³, SI-correct: [`:370`](http://localhost:8000/winisd/winisd_tools/scrapers/scrapers/lib/model_wdr.py#L370), [`:457`](http://localhost:8000/winisd/winisd_tools/scrapers/scrapers/lib/model_wdr.py#L457), [`:486`](http://localhost:8000/winisd/winisd_tools/scrapers/scrapers/lib/model_wdr.py#L486) | [`:380`](http://localhost:8000/winisd/openisd/packages/winisd/src/driver.ts#L380) — m³, SI-correct                                | m³, SI-correct: `Vd` [`:171-172`](http://localhost:8000/winisd/openisd/packages/engine/src/driver.ts#L171-L172), `Xmax` [`:160-161`](http://localhost:8000/winisd/openisd/packages/engine/src/driver.ts#L160-L161), `Sd` [`:166-167`](http://localhost:8000/winisd/openisd/packages/engine/src/driver.ts#L166-L167) |
...
constants** for the SPL-from-efficiency step (WINISD_SCHEMA's inferred ≈109, `winisd/driver.ts`'s
...
> _"100% retire calc code in python and reimpl it js. python will call js for all calcs. one
> source of truth. calcs will live in openisd. units MUST be SI throughout up until point of
> use."_ — [TODO.md:2407-2409](http://localhost:8000/winisd/winisd_tools/TODO.md#L2407-L2409)

> _".wdr is generated ONLY on demand… by calling a JS `ymlToWdr()`/equivalent lib function…
> the Python pipeline invokes the JS lib as an external program (subprocess call to a Node CLI
> wrapping the same function the UI calls)"_ — [BACKLOG.md:33-40](http://localhost:8000/winisd/openisd/BACKLOG.md#L33-L40)
...
- Design doc states the exact rule the human just restated independently:
  _"C = the field has a value and was not entered (derivable, given the E-set); E = the human
  entered it ← the only fact not recoverable from the values"_ —
  [docs/design/DRIVER_ADT_DESIGN.md:28-32](http://localhost:8000/winisd/openisd/docs/design/DRIVER_ADT_DESIGN.md#L28-L32).
...
- Carried into the file format correctly: `Driver.raw()` **excludes C fields on principle** —
  _"Computed (C) fields are deliberately excluded: raw() is 'what was entered'"_ — so `.owdr`'s
...
- WinISD's own file-level equivalent (`ParState`) is independently, correctly implemented on
  ...
  into the schema doc: _"A scraper that writes a fixed ParState for every driver… will mark
  computed fields like Qts, Vd, Dd as E — permanently pinning them"_ —
  [WINISD_SCHEMA.md:386](http://localhost:8000/winisd/openisd/docs/design/WINISD_SCHEMA.md#L386).

**The Xmax(Hc,Hg) case reaches `cell('Xmax')`.** `Driver#derive()` calls
[`solveConsistencyGroup(entered, { full: true })`](http://localhost:8000/winisd/openisd/packages/winisd/src/driver.ts#L426)
directly and [`cell()`](http://localhost:8000/winisd/openisd/packages/winisd/src/driver.ts#L185-L196)
returns `'C'` for any field that solve filled, so a driver carrying `Hc`/`Hg` but no `Xmax` shows
`state: 'C'` — the mark WinISD writes for the same inputs (probe `A_hchg_only`,
[`runs/xmax_route.jsonl`](http://localhost:8000/winisd/winisd_research/runs/xmax_route.jsonl)).
`deriveDriver` is called separately for `.errors` only; the full solver, not that call, is what
supplies the value.
...
naming root only. This means BACKLOG.md's own line 25, _"the app reads `openisd.yml` only,"_ is
...
1. **§2 row 18** — which of the three SPL/no constants (109 inferred, 112.1, 112.2) is correct,
   and why do three exist? Needs a dedicated probe against `drivers/mysamples/`-style empirical
...
`packages/winisd/src/driver.ts` implements the `Driver` class — `enter`/`clear`/`fromWdr`/
per-field marks — wired live into `store.ts`. See
[`PLAN_OPENISD_DRIVER_MODEL.md`](PLAN_OPENISD_DRIVER_MODEL.md) for its current status
(AD-8 now condemns the class itself in favour of `OpenISDDriver`).

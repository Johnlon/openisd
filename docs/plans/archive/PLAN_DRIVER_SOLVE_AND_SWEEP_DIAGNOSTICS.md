...
**Status: COMPLETE (2026-09-20). S1–S7 landed 2026-09-17 (`6161b6c`..`713dbf3`). S8 was dropped by QO167 (calculated values stay in every saved file, load recomputes). S10 landed 2026-09-20 (sealed joins cascade via loss-aware solve). Browser suite 289/289 green 2026-09-20.** 2026-09-16 rewrite: dropped the accreted
...
The sweep can silently fail: an unsized vent port (no `tuning_goal_hz`, no `length_m`) produced
...
| S7-a | 2026-09-16 | **`Field` is the ubiquitous model type** (John). Every solvable quantity — driver spec AND box fields (`vented.tuning_goal_hz`/`length_m`, `passiveRadiator.tuning_goal_hz`/`addedMass_kg`, sealed `Qtc`/`volume_m3`, …) — is stored as the same `{state:'C'\|'E', value}` entry and read through the same `Field` lens. Plain `number \| null` box fields die. |
...
- Box fields join: `vent.tuning_goal_hz`/`vent.length_m`/`passiveRadiator.tuning_goal_hz`/`addedMass_kg`
...
| S10 | Sealed joins the cascade (J4) | **DONE (2026-09-20)** — `solveSealedAlignment` takes `Ql`/`Qa` (losses) as `SolverInput`s and reproduces `#sealedQtc`; sealed box wired into `#resolve()`; `q_tc` readout reads entry state 'C'; verified by domain & consistency tests. |
...
| J5 | **`systemTuning_hz` semantics changed** in S2-7d2: old code solved it with unstated mass = 0 ("an unreachable request never drags the value"); now it is `solvePr`'s unified output, so an unreachable `tuning_goal_hz` target yields a negative-mass-derived value + DQ, same as `resonanceWithAddedMass_hz`. Also: the cascade solves ONLY the active box type's vent/PR pair. | Consistent with "redline all the fields"; accepted unless John objects. |

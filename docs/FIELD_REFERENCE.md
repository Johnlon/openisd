...
**OpenISD deviates deliberately: `power_peak_W` is a field WinISD has no slot for.** WinISD's
driver schema carries one power rating, `Pe`, the continuous/thermal figure — no peak/short-term
rating. OpenISD adds `power_peak_W` for the datasheet's separate peak number. It is never read
from or written to a `.wdr`/`.wpr` file: a driver imported from WinISD reads it as not-entered,
and a driver saved back to `.wdr` never carries it.

**Neither `numVC` nor `VCCon` is a simulation input.** In WinISD the wiring combo performs a ONE-SHOT REWRITE of the
...
| `tuning_goal_hz` | Hz | The port's tuning frequency, `Fb`. |
...
## Defaults are stored, never applied at read time

A field with a default holds that default in the record as a `C` entry, written by the resolve
that derived it. Nothing is defaulted on the way out. A reader that sees no entry is reading an
unresolved record, not a field silently standing in for a value it never stated.

So every field answers the same three questions the same way: `E` the project stated it, `C`
something derived it, absent nobody has. `VCCon` carries `C` parallel, `numVC` and a vent's
`count` carry `C` 1, and the environment trio carries `C` at the app's Options values — each
one visible in the file and marked in the editor like any other cell. (John, 2026-09-24:
"simply no reason for these exceptions to the rule.")

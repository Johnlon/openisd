...
|---|---|---|
| Sealed | Alignment dropdown: `0.500 Critically damped`, `0.577 Max flat delay response`, `0.707 Max flat amplitude response` *(default)*, `0.800`, `0.900`, `1.000`, `1.100`, `1.200`, `1.500` `Equal ripple response` | This is one numeric target-Qtc menu, not separate Bessel/Chebyshev formula families. `sealedFromQtc()` exists (`boxDesign.ts:54`, `Vb = Vas / ((Qtc/Qts)² − 1)`); the engine still needs the typed option catalog, inverse Qtc-from-Vb and closest-option lookup. |

...
> **Archived in place.** `ARCHITECTURE.md` AD-8 condemns the `Driver` class this document
...
provenance ADT. An Entered value that no longer reconciles with a freshly-typed sibling is
surfaced via `checkConsistency()` as a DQ mark, never silently cleared or overridden; see

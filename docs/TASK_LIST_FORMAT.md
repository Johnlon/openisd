...
## Granularity — one line per action, not per file or feature

Maintain the running task list (`todowrite`) at the FINEST granularity the work has. Break
each feature/bug into its concrete steps (implement X, verify Y, run Z, remove the temporary
debug log, run the regression) so "what are we working on?" is answerable from the list alone.
A reader with no context must be able to see exactly what is being worked on, in what order,
and what each item means.

Keep one item `in_progress` at a time. Distinguish BLOCKING work from DEFERRED/background
items (e.g. pending human rulings, another session's files), and say so in the list.

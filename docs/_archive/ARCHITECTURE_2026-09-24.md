...
graph LR
    ...
    subgraph app["OpenISD — browser SPA"]
        APP["Vue 3 + TypeScript app<br/>packages/ui<br/>consumes @openisd/design · @openisd/persistence"]
    end

    PROJ_PKG["<b>@openisd/design (bridge)</b><br/>headless core — no DOM, no browser<br/>one bundled file, loadable by embedded V8<br/>driver.yml to openisd.yml · openisd.yml to .wdr"]
    ...
    PROJ_PKG -. " shares domain + engine with " .-> APP
    TOOLS -->|" writes openisd.yml + .wdr "| DRIVERS
    DRIVERS -. " openisd.yml published as " .-> BUNDLE
```

**The utility is a SEPARATE ARTIFACT, not the browser app.** `winisd_tools` embeds V8 and loads a single bundled
JavaScript file; it cannot load a Vue application, and it must not have to. So the projection bridge is built from the
headless package — `@openisd/design` (`packages/design/winisd/bridge.ts` → `dist/openisd-bridge.js`) — which depends on
nothing that touches a DOM, a `window`, a network or a filesystem. It builds to one file with no external imports at
runtime, because that is what an embedded interpreter can load.

The browser SPA and the Python pipeline therefore share the DOMAIN and the ENGINE in `@openisd/design`, and each brings its own shell: the SPA
brings Vue (`packages/ui`), `winisd_tools` brings V8. Neither depends on the other's shell.
...
graph TD
    ...
    subgraph L4b["DOMAIN — headless, no DOM, no browser"]
        MODEL["<b>@openisd/design</b><br/>packages/design/domain/<br/><b>OpenISDProject</b> and its members:<br/>OpenISDDriver · OpenISDPassiveRadiator<br/>OpenISDBox · OpenISDVent · OpenISDEnvironment<br/>OpenISDSignal · OpenISDFilter · OpenISDListening<br/><i>pure data. No DOM, no browser, no Vue.</i>"]
        ENGINE["<b>@openisd/design/engine</b><br/>packages/design/engine/<br/>derive · sweep · circuit<br/>alignments · filters"]
    end

    subgraph L4c["IO — the serialization / file boundary"]
        SERIAL["<b>@openisd/design/winisd</b><br/>packages/design/winisd/<br/>.wdr · .wpr · ParState"]
    end
...
graph TD
    subgraph STORES["APPLICATION — the mutable state layers (packages/ui/src/logic/)"]
        ...
        PS["<b>PresentationState</b><br/>logic/presentationState.ts<br/><i>panels · per-chart zoom · units</i>"]
    ...
    PROJ["<b>OpenISDProject</b> — @openisd/design<br/>superset of a .wpr · holds the ENTIRE UI data<br/><i>dormant data is KEPT, never deleted</i>"]
...
`OpenISDProject` itself is a public class (`packages/design/domain/openisdDomain.ts`) — the store names its type for a
...
(`packages/design/engine/types.ts`) — there is no 6th-order and no ABC anywhere in the code.
...
graph TD
    ...
    SVC["<b>SERVICES</b><br/>packages/persistence<br/>db/ · diagnostics/ · logging/"]
    MODEL["<b>@openisd/design</b><br/>OpenISDDriver · OpenISDProject"]
    ENGINE["<b>@openisd/design/engine</b><br/>physics"]
    WINISD["<b>@openisd/design/winisd</b><br/>serialisation"]
...
| Module                                                                | Layer   |
|-----------------------------------------------------------------------|---------|
| `packages/persistence` (`@openisd/persistence`)                      | service |
| `diagnostics/selftest.ts` · `logging/flash.ts`                        | service |
| `@openisd/design` (`domain/` · `engine/` · `winisd/`)                 | domain  |

**CONDEMNED** — scheduled for deletion, still imported.

| Module                 | Replaced by                                                            |
|------------------------|------------------------------------------------------------------------|
| `Driver` ADT (deleted) | `OpenISDDriver`, a member of `OpenISDProject`, behind `ManagedProject` |
...
| `ManagedOpenISDProject` file IO (QO78) | `.wdr`/`.owdr`/`.wpr` codec methods on the facade itself (`exportDriverWdr`/`exportDriverOwdr`/`exportWpr`/`importWpr`/`persistedDriverText`/`loadDriverFrom*Text`) — the driver crosses the boundary only as serialised text/bytes. No `.owpr` codec: its on-disk shape is `SerializedState`, an A8 decision | `@openisd/design`, `@openisd/design/winisd`, `wprMapping.ts`, `driverFileText.ts`, `fileFormat.ts`, `persist.ts`'s gzip helpers |
| `managedDriver.ts` (QO78)              | Driver file IO NOT bound to a project — disk/library rows → an `OpenISDDriver`                                                                                                                                                                                                                                | `@openisd/design`                                                                                                       |
...
**Serialisation is a leaf.** `@openisd/design/winisd` turns the OpenISD record into WinISD's bytes and back. WinISD is a
...
|-------------------------|--------------------------------|--------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------|
| `@openisd/design/engine`| `packages/design/engine/`      | **The only place electro-acoustic maths exists** — driver derivation, circuit solve, sweeps, alignments, filters, physical constants | WinISD concepts (WDR, ParState), file formats, DOM, app state         |
| `@openisd/design`       | `packages/design/domain/`      | `OpenISDDriver`, `OpenISDProject` — the domain model, its provenance, its derivation. No separately exported record type              | File formats, DOM, app state                                          |
| `@openisd/design/winisd`| `packages/design/winisd/`      | Serialisation to and from WinISD's files: `.wdr`, `.wpr`, ParState, the 48-key order; embedded V8 bridge (`bridge.ts`)              | The driver model, derivation, live state, DOM, app state              |
| `@openisd/persistence`  | `packages/persistence/`        | Repositories over driver/PR/prefs/project collections and storage ports (`KeyValueStorage`, `FileStorage`)                          | App state, workflow, DOM beyond storage APIs                          |
...
| `driverRepo`            | `packages/persistence/` / `ui/src/db/` | The driver commons: index, search, filter, lookup. Answers questions, returns records                                        | App state, workflow, `.vue` imports                                   |
| `myDriverRepo`          | `packages/persistence/` / `ui/src/db/` | User-saved drivers: read, write, delete by identity                                                                          | App state, workflow, `.vue` imports                                   |
| `prefsStore`            | `packages/persistence/` / `ui/src/db/` | Browser-local preferences — favourites, session, layout                                                                      | App state, workflow, `.vue` imports                                   |
...
[`solveDriver`](packages/design/engine/solver.ts) in `packages/design/engine/solver.ts` is the one implementation;
...
`CalculationIssue`](packages/design/engine/consistency.ts) in
`packages/design/engine/consistency.ts`). The same mark covers an over-determined group, where more members are asserted
...
**The core has no browser.** `@openisd/design` (including its `domain/`, `engine/`, and `winisd/` subpaths) contains no DOM, no `window`, no
...
`@openisd/design`'s `ventArea_m2()` — correct — but then called directly from
...
Gated by
`packages/design/test/winisd/wdr-model-coverage.test.ts`.
...
| **`PresentationState`** | PRESENTATION state — which panel/dialog is open, per-chart zoom, unit tokens                                                                          | `packages/ui/src/logic/presentationState.ts` |
...
holds `newProjectOpen`, `browseOpen`, `editDriver`, `editDriverInfo`, `yRanges` and `ui` (unit tokens,
environment defaults, username, chart colours, per-shell layout) — moved out of the store's `state` object, which
now holds only persistent design state (`box`, `project`).

**The graph cursor, open charts and the sealed loss model are PROJECT-scoped, not presentation-scoped (QO130,
reversing QO90 for these fields — two open projects must not share one cursor).** They live on `OpenISDProject`
itself: `.cursorF`/`.pinnedF`/`.cursorLocked`/`.dragRange`/`.graphs`/`.lossMode`, reached through `ManagedProject`
like every other project field. `graphs`/`lossMode` persist (`.owpr`, autosave, share link) same as any other
project data. The four cursor fields go further under QO168: plain in-memory instance state, excluded from
EVERY saved record — not `.owpr`, not the autosave, not a share link — a documented exception in
`architecture-project-has-three-fields.test.ts`. `OpenISDProject.isModified()` also excludes the whole `charts`
slot (which holds `graphs`) from its unsaved-changes diff, same treatment as chart zoom/sweep range — see
"Round-trip chart view state" in `BACKLOG.md`.
...
`openisd.state` local save, and a share link — human ruling 2026-08-14: a link is a complete description of the
session, stripped of nothing, so the recipient sees the sender's open panels and unit choices too — except the
cursor, which QO168 keeps out of every saved record, share links included). `UrlAppState`

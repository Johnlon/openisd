...
| Step | Evidence |
| --- | --- |
| `prVas()` returns **litres** | `packages/engine/src/formulas.ts:15` — `return prCms * prSd * prSd * RHO * C * C * 1000;` The trailing `* 1000` is the m³→l conversion. |
...
| The default design exports a wrong number today | `P_DEFAULTS.prCms=0.0008`, `prSd=0.0133` (`packages/ui/src/logic/store.ts:43`) ⇒ `prVas` = **20.074**, written as `Vas=20.074…`. WinISD will read 20 m³. Correct value: `0.020074`. |
| The export is reachable from the UI | `packages/ui/src/ui/components/ExportMenu.vue:29` "Save As WinISD project (.wpr)" → `exportWpr()` → `packages/ui/src/logic/useDesignIO.ts:139-140`. |
...
| --- | --- | --- | --- |
| `[SignalSource].Rg` | literal `0.1` | `state.P.Rs` | `wpr.ts:187` `['Rg', signal.Rg ?? 0.1]`; `wprMapping.ts:54` passes `signal: { P: P.Pin }` — no `Rg` key, so the default always wins. `state.P.Rs` default is 0.1 but is user-editable (`OriginalShell.vue:1418`). |
| `[Box].alfaVC` | literal `0.0039` | `state.P.alfaVC` | `wpr.ts:175` `['alfaVC', 0.0039]`. Store holds SI 1/K (`store.ts:47`), same unit as the file (O-C line 93 `alfaVC=0.0039`) — so it is a pure pass-through that was never wired. |
...
`fieldRegistry.ts:398` declares `fLe … unit: 'kHz', min: 0, max: 100`, and its own description says
...
:precision="precision('fLe')">` (`DriverEditorModal.vue:561`) with **no `field=` prop**, and
...
`fieldRegistry.ts:404` — `no … unit: '%', min: 0, max: 100`. The model holds the fraction
...
Inert for the same reason as F4 — `DriverEditorModal.vue:616` passes no `field=`.

### ⚠ INERT F6 — the "define a new driver" dialog drops 13 fields and stores dimensions in mm

`packages/ui/src/ui/components/DriverDefineModal.vue` (Modern skin only — reached via
`DriverPanel.vue:160` → `SidePanel` → `ModernShell`).

- `applyDriver()` (lines 302-318) emits **only** `Fs Qts Qes Qms Re Le Z Vas Sd Xmax Pe` plus the
  8 `dimOnly` keys. Typed values for `fLe KLe numVC VCCon alfaVC Rt Ct c roo Dia Vd no SPL`
  (declared at lines 48-68) are **discarded**.
- The 8 dimension fields are emitted under `outerMm basketMm magnetMm depthMm thickMm
  magDepthMm vcDiaMm dvol` — in **mm** and **litres** (line 29: *"no SI conversion, stored as-is
  in mm or g"*). The driver model's own keys are `outer basket magnet depth thick magnetDepth
  VCd basketDisplacement` in **metres / m³** (`driver.ts:87-113`). Nothing reads the `*Mm` keys,
  so the values vanish — and wiring them without converting would be a 1000× error.
...
| --- | --- | --- | --- |
| `advPressure` | `unit: 'kPa'`, precision 2 | **Pa**, precision 1 | `OriginalShell.vue:1403` and `OptionsModal.vue:194` both bind `group="pressure" base="Pa" :precision="1"`. WinISD also shows Pa (S-6, `101325.0`). The registry string is the odd one out. |
| `Hc`, `Hg`, `Xlim` | `unit: 'm'` | **mm** (`:scale="1000"`) | `DriverEditorModal.vue:583, 588, 599`. |
| `Gloss`, `SPLmaxLF`, `Mcost` | `modeled: false` + *"formula has NOT been recovered"* | the engine derives all three | `packages/engine/src/driver.ts:246` (`loss`), `:253-255` (`SPLmaxLF`), `:266-267` (`Mcost`). |
...
| --- | --- | --- |
| `no` fraction ÷100 in a code path | **Fixed.** Editor binds `:scale="100"` (fraction→%); the engine holds the fraction end-to-end. | `DriverEditorModal.vue:616`; `packages/engine/src/efficiency.ts:29-31, 45-47`; `command grep -rn '/ *100' packages/engine/src` finds no η₀ divide. WinISD shows `no … %` at 4 dp (S-1). |
| `Gloss` fraction-vs-percent | **Fixed during this audit, single-placed.** The ONE ×100 is the binding; the engine module states the percent "exists nowhere in this module". | `DriverEditorModal.vue:711` `:scale="100" :precision="precision('Gloss')"`; `packages/engine/src/driver.ts:242-246`; gated by `packages/ui/test/ui/driver-editor-units.test.ts:169-224`. WinISD shows `Gloss … %` 4 dp (S-2). |
| `fLe` `:scale="1000"` → should be `1e-3` | **Fixed.** | `DriverEditorModal.vue:556-562` (`:scale="1e-3"`, with the inverted-scale trap written into the comment); gated by `driver-editor-units.test.ts:132`. WinISD shows `fLe … kHz` 5 dp (S-1). But see **F4** — the registry bound is still in Hz. |
...
| Dimensions tab in metres under an `m` label; `Thick` metres under `in` | **Fixed.** All 8 fields `:scale="1000"` (mm) / `1e6` (cm³), with the old symptom recorded in the comment at line 736. | `DriverEditorModal.vue:738-745`; gated by `driver-editor-units.test.ts:158`. WinISD's own defaults are mixed — `Thick` in **in**, `Dvol` in **in^3**, the other six in **m** (S-3) — so mm/cm³ is a documented divergence within WinISD's own unit cycle, not an error. |
...
| `VCCon` 1=parallel / 2=series, writer defaulting to 2 | **openisd side is correct — the lead is upstream.** Every openisd writer defaults to **1**. | `packages/winisd/src/classic/wdr.ts:37` `'VCCon=' + (raw.VCCon \|\| 1)`; `openisdToWdr.ts:42` `['VCCon', 1]`; `driver.ts:316-321` backfills `'1'`; editor select `1=Parallel / 2=Series` (`DriverEditorModal.vue:641`). Matches WDR_SCHEMA §3.5 and WinISD's own `Connection: Parallel` default (S-1). The "defaults to 2" writer is `winisd_tools/scrapers/plugins/visaton/emit.py:119-130`, **outside this repo**. |
| `AlfaVC` labelled `1000/K` but bound with no `:scale` | **Fixed in both places.** | `DriverEditorModal.vue:654` `:scale="1000"`; `OriginalShell.vue:1207` `group="tempCoeff" base="perMilliK"` (factor 1000, `units.ts:93-97`). Gated by `driver-editor-units.test.ts:140`. WinISD shows `AlfaVC … 1000/K` 4 dp (S-2). |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 8 | `Qts` | — | — | `1`, 3 dp | — 3 dp | ✅ | O-A `Qts=3`; editor L488 |
| 9 | `Znom` | Ω | Ω (`Z`) | `1`, 3 dp, `ohm` | `ohm` 3 dp | ✅ | O-A `Znom=8`; L621 |
| 10 | `Fs` | Hz | Hz | `1`, 2 dp, `Hz` | `Hz` 2 dp | ✅ | O-A `Fs=4`; L493 |
| 11 | `Pe` | W | W | `1`, 2 dp, `W` | `W` 1 dp | ◐ dp only | O-A `Pe=12`; L604 |
| 12 | `SPL` | dB/W/1m | dB | unscaled, `dB` | `dB` 2 dp | ✅ unit; ❌ **F2** on save | O-A `SPL=61.605…` reproduced by `efficiency.ts:45`; L632 |
| 13 | `Re` | Ω | Ω | `1`, 3 dp, `ohm` | `ohm` 3 dp | ✅ | O-A `Re=6`; L529 |
| 14 | `Le` | **H** | H | `1000`, 3 dp, `mH` | **`H`** 6 dp | ✅ (documented divergence) | O-A `Le=7`; O-E `'3.0'`→`0.003` "voice-coil inductance (H)"; L546 |
| 15 | `fLe` | **Hz** | Hz | `1e-3`, 5 dp, `kHz` | `kHz` 5 dp | ✅ value / ⚠ **F4** bound | O-A `fLe=8`; DOC §3.2; L561 |
| 16 | `KLe` | H·√Hz | H·√Hz | unscaled, 2 dp | `H*sqrt(Hz)` 6 dp | ◐ dp only | O-A `KLe=49.628…`; L566 |
| 17 | `BL` | T·m | T·m (`Bl`) | `1`, 3 dp, `Tm` | `Tm` 5 dp | ◐ dp only | O-A `BL=0.868…`; L535 |
| 18 | `Xmax` | **m** | m | `1000`, 3 dp, `mm peak` | **`m`** 3 dp `peak` | ✅ (documented divergence) | O-A `Xmax=0.009`; O-E "stored in m, not mm"; L577 |
| 19 | `Cms` | **m/N** | m/N | `1000`, 4 dp, `mm/N` | **`um/N`** 1 dp | ✅ (documented divergence) | O-A `Cms=0.3166…`; O-E "(m/N)"; L517 |
| 20 | `Qms` | — | — | `1`, 3 dp | — 3 dp | ✅ | O-A `Qms=2`; L483 |
| 21 | `Qes` | — | — | `1`, 3 dp | — 3 dp | ✅ | O-A `Qes=1`; L478 |
| 22 | `Rms` | kg/s ≡ N·s/m | kg/s | `1`, 4 dp, `Ns/m` | `Ns/m` 5 dp | ✅ (same unit, two spellings) | O-A `Rms=0.06283…` = 2π·4·0.005/2; O-E "(kg/s)"; L523 |
| 23 | `Mms` | **kg** | kg | `1000`, 2 dp, `g` | **`kg`** 5 dp | ✅ (documented divergence) | O-A `Mms=0.005`; O-E "stored in kg, not g"; L511 |
| 24 | `Sd` | **m²** | m² | `1e4`, 2 dp, `cm²` | **`m^2`** 4 dp | ✅ (documented divergence) | O-A `Sd=0.0017754…`; O-E "stored in m², not cm²"; L552 |
| 25 | `Vas` | **m³** | m³ | `1000`, 2 dp, `L` | **`in^3`** 1 dp | ✅ (mm/L divergence documented) | O-A `Vas=0.14158…`; O-E "stored in m³, not litres"; L499 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 27 | `Vd` | **m³** | m³ | `1e6`, 2 dp, `cm³` | `cm^3` 0 dp (S-1) | ✅ unit; ❌ **F2**; ◐ dp | O-A `Vd=1.5979e-5` = Sd×Xmax; DOC §3.4 "displays as 200 cm³"; L594 |
| 28 | `no` | **fraction** | fraction | `100`, 2 dp, `%` | **`%`** 4 dp (S-1) | ✅ unit; ❌ **F2**; ⚠ **F5**; ◐ dp | O-A `no=8.812e-6` reproduced by `(4π²/c³)·Fs³·Vas/Qes` = 8.81202645607949e-6; L616 |
| 29 | `Dd` | **m** | m | `1000`, 2 dp, `mm` | **`m`** 3 dp (S-1) | ✅ unit; ❌ **F2** | O-A `Dd=0.047545…` = 2√(Sd/π); O-E "stored in m, not mm"; L541 |
| 30 | `EBP` | Hz | Hz | read-only, 1 dp, `Hz` | `Hz` 2 dp (S-2) | ✅ | O-A `EBP=4` = Fs/Qes; `DriverEditorModal.vue:705` |
| 31 | `numVC` | integer | integer | unscaled, 2 dp | integer (S-1) | ✅ unit; ◐ dp (shows `1.00`) | O-B `numVC=1`; `driver.ts:445`; L637 |
| 32 | `Hc` | **m** | m | `1000`, 2 dp, `mm` | **`m`** 3 dp (S-1) | ✅ value; ⚠ F8 registry says `m` | O-A `Hc=10`; O-E `Hc_mm` "(mm)" → the RECORD is mm, the FILE is m, converted at `openisdToWdr.ts:60` `1e-3` ✅; L583 |
| 33 | `Hg` | **m** | m | `1000`, 2 dp, `mm` | **`m`** 3 dp | ✅ value; ⚠ F8 | O-A `Hg=9.982`; `openisdToWdr.ts:60`; L588 |
| 34 | `SPLmax` | dB | dB | unscaled, `dB` | `dB` 2 dp (S-2) | ✅ unit; ❌ **F2** | O-A `SPLmax=69.397…` = SPL+10log₁₀(12); `driver.ts:235` |
| 35 | `SPLmaxLF` | dB | dB | unscaled, `dB` | `dB` 2 dp (S-2) | ✅ unit; ❌ **F2**; ⚠ F8 stale registry | O-A `SPLmaxLF=64.635…`; `driver.ts:253-255` |
| 36 | `USPL` | dB/2.83V | dB | unscaled, `dB` | `dB` 2 dp (S-1) | ✅ unit; ❌ **F2** | O-A `USPL=62.859…` = SPL+10log₁₀(8/6); `driver.ts:199` |
| 37 | `alfaVC` | **1/K** | 1/K (`tc`) | `1000`, 2 dp, `1000/K` | **`1000/K`** 4 dp (S-2) | ✅ unit; ◐ dp | O-C `alfaVC=0.0039` = copper 0.0039/K; L654; test `driver-editor-units.test.ts:140` |
| 38 | `Rt` | K/W | K/W (`Rth`) | unscaled, `K/W` | `K/W` 5 dp (S-2) | ✅ unit; ◐ dp | O-A `Rt=0`; L659 |
| 39 | `Ct` | J/K | J/K (`Cth`) | unscaled, `J/K` | `J/K` 5 dp (S-2) | ✅ unit; ◐ dp | O-A `Ct=0`; L664 |
| 40 | `gamma` | m/(s²·A) ≡ N/(A·kg) | same | unscaled, `N/(A·kg)` | `N/(A*kg)` 5 dp (S-2) | ✅ unit; ❌ **F2**; ◐ dp | O-A `gamma=173.664…` = BL/Mms; `driver.ts:230` |
| 41 | `Rme` | N·s/m | N·s/m | unscaled, `Ns/m` | `Ns/m` 5 dp (S-2) | ✅ unit; ❌ **F2**; ◐ dp | O-A `Rme=0.12566…` = 2π·4·0.005/1; `driver.ts:219` |
| 42 | `Mpow` | N/√W | N/√W | unscaled, `N/√W` | `N/sqrt(W)` 5 dp (S-2) | ✅ unit; ❌ **F2**; ◐ dp | O-A `Mpow=0.35449…` = √Rme; `driver.ts:228` |
| 43 | `Mcost` | N·s/m (DOC) / kg/s (S-2) | same quantity | unscaled, `kg/s` | `kg/s` 5 dp (S-2) | ❔ **the two spellings are not the same quantity** — DOC §3.4 says N·s/m, WinISD's pane says kg/s. N·s/m ≡ kg/s dimensionally, so this is a spelling difference, but no probe settles which WinISD means. ⚠ F8 stale registry | O-A `Mcost=0.125777…`; `driver.ts:266-267`; DOC §3.4 vs S-2 |
| 44 | `Gloss` | **fraction of Xmax** | fraction (`loss`) | `100`, 4 dp, `%` | **`%`** 4 dp (S-2) | ✅ (fixed this session) | O-A `Gloss=1.72503712771898` reproduced to 14 s.f. by `g/((2π·Fs)²·Xmax)`; L711; test `driver-editor-units.test.ts:169-224`. See **F9**. |

### 3.5 Voice-coil connection (position 45)

| # | Key | File | SI/store | Display | WinISD | Verdict | Test run |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 45 | `VCCon` | 1=parallel, 2=series | same integer | `<select>` 1/2 | `Connection: Parallel` (S-1) | ✅ | O-A/O-B/O-C all `VCCon=1`; `wdr.ts:37`, `openisdToWdr.ts:42`, `driver.ts:318`; `DriverEditorModal.vue:641` |

### 3.6 Environment constants (positions 46–47)

| # | Key | File | SI/store | Display | WinISD (S-2) | Verdict | Test run |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 46 | `c` | m/s | m/s | read-only `C.toFixed(2)` `m/s` | `m/s` 2 dp | ✅ | O-A `c=343.684120962152`; `DriverEditorModal.vue:720` |
| 47 | `roo` | kg/m³ | kg/m³ | read-only `RHO.toFixed(5)` `kg/m³` | `kg/m^3` 5 dp | ✅ | O-A `roo=1.20095217714682`; `DriverEditorModal.vue:724` |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 48 | `Thick` | m | m (`thick`) | `1000` `mm` | **`in`** 2 dp | ✅ | O-B `Thick=0`; O-E `thick_mm` "(mm)" → `openisdToWdr.ts:61` `1e-3` ✅; editor L738 |
| 49 | `Depth` | m | m (`depth`) | `1000` `mm` | `m` 3 dp | ✅ | O-E `depth_mm` `'3.875'`→`98.425` mm; `openisdToWdr.ts:61`; L739 |
| 50 | `MagDepth` | m | m (`magnetDepth`) | `1000` `mm` | `m` 3 dp | ✅ | O-E `magnet_depth_mm`; `openisdToWdr.ts:62`; L740 |
| 51 | `Magnet` | m | m (`magnet`) | `1000` `mm` | `m` 3 dp | ✅ | O-E `magnet_dia_mm` `24.5` mm; `openisdToWdr.ts:62`; L741 |
| 52 | `Basket` | m | m (`basket`) | `1000` `mm` | `m` 3 dp | ✅ | O-E `basket_dia_mm` `232.0` mm; `openisdToWdr.ts:63`; L742 |
| 53 | `Outer` | m | m (`outer`) | `1000` `mm` | `m` 3 dp | ✅ | O-E `outer_dia_mm` `203.2` mm; `openisdToWdr.ts:63`; L743 |
| 54 | `Vcd` | m | m (`VCd`) | `1000` `mm` | `m` 3 dp | ✅ | O-E `voice_coil_dia_mm` `38.1` mm; `openisdToWdr.ts:60`; L744 |
| 55 | `DVol` | **m³** | m³ (`basketDisplacement`) | `1e6` `cm³` | **`in^3`** 1 dp | ✅ | O-E `driver_volume_l` `1.6` **litres** → `openisdToWdr.ts:64` `1e-3` ✅; L745 |
...
| — | `Xlim` | **no file key** — ParState slot 10 only | m | `1000` `mm` | `m` 3 dp (S-1) | ✅ (openisd writes an extra `Xlim=` line for its own round-trip only — `wdr.ts:42`) | O-D `s-xlim.wdr` writes no `Xlim=`; DOC §8.4 pos 11; O-E `Xlim` "stored in m"; L599 |
...
| `endcorrection` | coefficient | coefficient | ✅ | O-C `endcorrection=0.732`; `fieldRegistry.ts:74-78`; `wpr.ts:140` |
...
| --- | --- | --- | --- | --- | --- | --- |
| `Vas` | **m³** | derived, **litres** | `L` | **`l`** | ❌ **F1 — ×1000** | O-C `Vas=0.0048` vs S-4 `4.80 l`; `formulas.ts:15`; `wprMapping.ts:101` |
...
| `Sd` | **m²** | m² (`P.prSd`) | `cm²` (`group="area" base="cm2"`) | **`cm^2`** | ✅ | O-C `Sd=0.0095` vs S-4 `95.0 cm^2` (×1e4); `OriginalShell.vue:1342`; `wprMapping.ts:104` |
| `Xmax` | **m** | m (`P.prXmax`) | `mm` (`group="length" base="mm"`) | **`mm`** | ✅ | O-C `Xmax=19` vs S-4 `19000.0 mm` (×1000 ⇒ the file is metres; that project's value is simply an absurd 19 m) ; `OriginalShell.vue:1345`; `wprMapping.ts:105` |
| `Me` | ❔ kg assumed | kg (`P.prMadd`) | `g` (`group="mass" base="g"`) | **`g`** | ❔ UNVERIFIED — every corpus value is 0, so kg-vs-g cannot be settled | O-C `Me=0`; S-4 `Added mass to cone 0.0 g`; `OriginalShell.vue:1357`; `wprMapping.ts:106` |

---

## 5. The sweep — store (`UiParams`) and the Original skin's panels

The store **always holds SI** (`units.ts` header; `store.ts:27-58` `P_DEFAULTS`). Two display
...
| --- | --- | --- | --- | --- | --- |
| `Vb` | m³ (`0.030`) | `group="volume" base="L"` ⇒ ×1000 | `l` | ✅ | `store.ts:28`; `OriginalShell.vue:1032`; `units.ts:47-51` |
| `Vf` | m³ | `group="volume" base="L"` | `l` | ✅ | `OriginalShell.vue:1076` |
| `Fb`, `Frc`, `boxResonance`, `rearResonance`, `portResonance`, `prFs`, `prFsMass` | Hz | `group="freq" base="Hz"` ⇒ ×1 | `Hz` | ✅ | `OriginalShell.vue:1040-1047, 1060-1066, 1085, 1294, 1321, 1332`; `units.ts:62-65` |
| `ventD`, `ventW`, `ventH`, `ventL` | m (`0.05`, `0.10`) | `group="length" base="cm"` ⇒ ×100 | `cm` (Vents pane) | ✅ | `store.ts:28`; `OriginalShell.vue:1252-1283`; `units.ts:52-56` |
| `ventCrossArea` (`ventArea`) | m² | `group="area" base="m2"` ⇒ ×1 | `m^2` | ✅ | `OriginalShell.vue:1291`; `fieldRegistry.ts:133-137` |
| `endCorrection` | coefficient | `<select>` of `END_CORRECTION_OPTIONS` | 0.732 default | ✅ | `fieldRegistry.ts:74-78`; `OriginalShell.vue:1239-1241`; O-C `endcorrection=0.732` |
| `Ql`, `Qa`, `Qp` | — | `:scale="1"` | — | ✅ | `store.ts:28`; `OriginalShell.vue:1450-1452` |
...
| `driveV` | V | `:scale="1"`, 2 dp, `V` | `V` 1 dp (S-5) | ✅ unit; ◐ dp | `OriginalShell.vue:1417`; `formulas.ts:47` `√(Pin·Re)` |
| `Rs` | Ω | `:scale="1"`, 3 dp, `ohm` | `ohm` 3 dp (S-5) | ✅ | `OriginalShell.vue:1418` |
| `listenDistance` | — | hardcoded `1.000`, disabled, `m` | `m` 3 dp (S-5) | ⚠ INERT (declared `modeled: false`, honestly shown as disabled) | `OriginalShell.vue:1410`; `fieldRegistry.ts:210-213` |
| `listenAngle` | — | hardcoded `0.0000`, disabled, `rad` | `rad` 4 dp (S-5) | ⚠ INERT | `OriginalShell.vue:1411`; `fieldRegistry.ts:215-218` |
| `genHz` | Hz | `v-model.number`, `Hz` | `Hz` 2 dp (S-6) | ✅ | `OriginalShell.vue:957` |
| `tempK` | K | `group="temp" base="K"` (affine, offset −273.15 for °C) | `K` 2 dp (S-6) | ✅ | `store.ts:57`; `OriginalShell.vue:1401`; `units.ts:72-76` |
| `humidityPct` | **percent** | plain `%` input | `%` 4 dp (S-6) | ✅ (the `.wpr` fraction is converted once — see §4.1) | `store.ts:57`; `OriginalShell.vue:1428`; `types.ts:225` |
| `pressurePa` | Pa | `group="pressure" base="Pa"` ⇒ ×1 | `Pa` 1 dp (S-6) | ✅ value; ⚠ **F8** registry says `kPa` | `OriginalShell.vue:1403`; `units.ts:78-82`; `fieldRegistry.ts:254-257` |
| `advSoundVelocity` | m/s | read-only 2 dp, `m/s` | `m/s` 2 dp (S-6) | ✅ | `OriginalShell.vue:1433`; `packages/engine/src/air.ts` |
| `advAirDensity` | kg/m³ | read-only 5 dp, `kg/m³` | `kg/m^3` 5 dp (S-6) | ✅ | `OriginalShell.vue:1434` |
| `vcTempRise` | K | `group="tempDiff" base="K"` ⇒ ×1 | — (no WinISD field) | ✅ | `store.ts:47`; `OriginalShell.vue:1206`; `units.ts:86-89` |
| `alfaVC` (project) | **1/K** | `group="tempCoeff" base="perMilliK"` ⇒ ×1000 | `1000/K` (S-2) | ✅ | `store.ts:47` `0.0039`; `OriginalShell.vue:1207`; `units.ts:93-97` |
| `driverAddedMass` | kg | `group="mass" base="g"` ⇒ ×1000 | `kg` on the Classic mock; `g` on the PR pane (S-4) | ✅ | `store.ts:47`; `OriginalShell.vue:1208`; `units.ts:66-70` |
| `prSd` | m² | `group="area" base="cm2"` ⇒ ×1e4 | `cm^2` (S-4) | ✅ | `store.ts:43`; `OriginalShell.vue:1342` |
| `prXmax` | m | `group="length" base="mm"` ⇒ ×1000 | `mm` (S-4) | ✅ | `store.ts:43`; `OriginalShell.vue:1345` |
| `prMadd` | kg | `group="mass" base="g"` ⇒ ×1000 | `g` (S-4) | ✅ | `store.ts:43`; `OriginalShell.vue:1357` |
| `prVas` (derived) | **litres** | `fmtU(prVas / 1000, …, 'volume', 'L')` — divided back to m³, then ×1000 | `l` (S-4) | ✅ on screen (the ÷1000 is deliberate); ❌ on `.wpr` export (**F1**) | `OriginalShell.vue:1337`; `formulas.ts:15` |
| `prMmd`, `prCms`, `prRms` | kg, m/N, kg/s | `:scale="1000"`/`1000`/`1` in `PRPanel.vue:179-189` (Modern) and `PREditModal.vue` | not shown by WinISD (it exposes Fs/Qms/Vas) | ✅ | `store.ts:43`; `PRPanel.vue:179-189` |
| `prNum` | integer | `:scale="1"` | integer (S-4) | ✅ | `OriginalShell.vue:1330` |
| `prFp` | Hz | solved by `usePrGroup` | `Fs (with added mass)` Hz (S-4) | ✅ | `store.ts:37`; `alignments.ts:113-118` |
...
| `fmin`, `fmax`, `N` | Hz, Hz, count | chart axis only | — | ✅ | `store.ts:44`; `sweep.ts:130` |
| `rgAtDriverSide`, `tlPortModel`, `forceFlatResponse`, `splXmaxLimited`, `ignoreHumidityAndPressure`, `circuitModel` | booleans / enum | checkboxes in `AdvancedOptions.vue` | checkboxes (S-6) | ✅ (no unit) | `AdvancedOptions.vue`; `store.ts:52` |
...
| | |
| --- | --- |
| Fields swept | **116** (56 `.wdr` keys + 27 `.wpr` keys + 41 store/panel/chart/record rows) |
| ❌ live mismatches | **3** (F1, F2, F3) touching **15 fields** |
| ⚠ inert / latent traps | **6** (F4, F5, F6, plus the boilerplate `.wpr` and `Dia`/`DateModified` rows) |

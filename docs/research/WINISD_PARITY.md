...
| :--- | :--- | :--- | :--- |
| **$M_{ms}$, $BL$, $Q_{es}$ Derivation** | Overwrites entered $M_{ms}$, $BL$, and $R_{ms}$ using derived WinISD formulas ($M_{ms} = \frac{\rho_0 c^2 S_d^2}{(2\pi F_s)^2 V_{as}}$, $BL = \sqrt{\frac{2\pi F_s R_e M_{ms}}{Q_{es}}}$). | Uses entered datasheet values directly; flags parameter conflicts via Data Quality (DQ). | Check **Use WinISD driver calculations** on Advanced tab or click **Reset to WinISD** (or clear entered $M_{ms}$, $BL$, $Q_{es}$ in Driver Editor). |

...
| :--- | :--- | :--- | :--- |
| **$M_{ms}$, $BL$, $Q_{es}$ Derivation** | Overwrites entered $M_{ms}$, $BL$, and $R_{ms}$ using derived WinISD formulas ($M_{ms} = \frac{\rho_0 c^2 S_d^2}{(2\pi F_s)^2 V_{as}}$, $BL = \sqrt{\frac{2\pi F_s R_e M_{ms}}{Q_{es}}}$). | Uses entered datasheet values directly; flags parameter conflicts via Data Quality (DQ). | Check **Use WinISD driver calculations** (`useWinisdDriverModel`) inside the WinISD Compatibility box on the Advanced tab, or click **Reset to WinISD** (or manually clear entered $M_{ms}$, $BL$, $Q_{es}$ in the Driver Editor). |
| **Drive Voltage ($e_g$)** | Includes series resistance $R_s$: $e_g = \sqrt{P(R_e + R_s)}$. | Matches WinISD behavior: $e_g = \sqrt{P(R_e + R_s)}$. | Applied automatically in the engine for all sweeps (no option required). |

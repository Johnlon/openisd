...
| **Drive Voltage ($e_g$)** | Includes series resistance $R_s$: $e_g = \sqrt{P(R_e + R_s)}$. | Previously used voice-coil power $e_g = \sqrt{PR_e}$. | Applied automatically in the engine for all sweeps. |

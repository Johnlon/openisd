...
| **Rg in the impedance chart** | The impedance chart includes Rg only when **Rg is at driver side** is on: off, Z is the driver alone and Rg changes nothing; on, Z = Z(off) + Rg exactly — measured 2026-09-26 by debugger on the W5-1138SMF at Rg 0 and 10 Ω ([records](http://localhost:8000/winisd/winisd_research/runs/sweep-w5-sealed-impedance-rg10-vcind1-driverside-off.json)). | Adds Rg to the impedance in both positions (`circuit.ts:140-145`) — [bug record](../../bugs/BUG_20260926_impedance-includes-rg-when-rg-is-not-at-driver-side.md). | Nothing a user can set matches it until the bug is fixed; with **Rg is at driver side** on the two agree. |

...
- [ ] **Original skin — port the remaining mock modals + interactions.** `OriginalShell.vue` Phase 0 wires the chrome, 2×2 layout, graph, six-type Box tab, and Driver/Filters/Signal/Project tabs to the shared store. Still to port from `mock/`: the 3-mode Driver Editor (project / My Drivers / standalone), the docked Tune panel, the full Vents tab (per-chamber vent geometry incl. ABC intrachamber vent), spinner drag-to-scrub on numeric fields, unit cycling, and the box-losses modal. Reuse shared components/composables where they exist (no forked logic). `[ui]`

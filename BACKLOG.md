...
- [ ] **P1 — IN PROGRESS (this session).** Unified project **Save / Save As / Export** on **all three skins** (Modern `AppHeader.vue`, `ClassicShell.vue`, `OriginalShell.vue`), all drawing from the single shared `useDesignIO()` composable. Three project actions, identical on every skin:
  - **Save / Save As → native OpenISD JSON** (`.openisd.json`) written to the **filesystem the user picks** via the File System Access API (`showSaveFilePicker`, handle retained so **Save overwrites in place**, Save As = new file), falling back to a plain download on Firefox/Safari (which lack the API). `localStorage` stays as crash-recovery only, never the record.
  - **Export → WinISD project `.wpr`.** New serializer `packages/winisd/src/wpr.ts` `toWpr()` (framework-free, per layering rule), emitting the 11 INI sections in fixed order + CRLF per `WINISD_WPR_FILE_SCHEMA.md`; reuses the driver ADT `toWdr()` for the field-identical `[Driver]` section. Box-type map: sealed→`BType 0`, vented→`1`, pr→`4`, bandpass4→`2`.
  - **Export → Share link** (existing `shareLink()`), plus the existing driver `.wdr` export kept in the Export group.
  - Also resolves: the Original skin's duplicate save button, and Classic's missing Share action.
  - **⚠ Verification gap (open decision for the human).** The repo has **only one** sample `.wpr` — a **passive-radiator** project (`docs/winisd/sample_project_Epique15_-_pr.wpr`). WPR output for **sealed / vented / bandpass** follows the documented schema but has **no in-repo file to byte-verify against** (notably sealed `[Box].Fr` semantics). Those paths are marked `⚠ unverified`. **To close this, the human can save a real sealed `.wpr` and a real vented `.wpr` from their WinISD into `docs/winisd/`** and the export can then be diffed against them. Even the PR `[Driver]` block can't be byte-identical to the sample (WinISD writes ~30 derived fields — `SPLmax`, `gamma`, `Rme`, `c`, `roo`… — OpenISD doesn't carry), so tests assert **structure** (section order, keys, CRLF, `[Box]`/`[PassiveRadiator]` values), not byte-equality. `[unit]` `[ui]`

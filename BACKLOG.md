...
- [ ] **P2** **Voice-coil thermal power compression** (WinISD parity — verified WinISD simulates it, `WINISD.md §12c`). **Covers TWO of the three tested fields — `Voice coil temp rise` AND `Voice coil resistance TC` (AlfaVC) — in one item, because they are a single physical effect: they act only as the product `AlfaVC·ΔT`, so neither does anything on its own and neither can be implemented without the other.** (The third tested field, `Added mass to cone`, is the separate item below.) At high drive the voice coil heats, its DC resistance rises, and output drops — a real, audible effect WinISD models and OpenISD currently ignores (it assumes a cold, constant `Re`). `[unit]` + `[ui]`

...
- [x] **BUG / P2** Clickable unit labels rotate but don't rescale the value → **fixed** with a reusable per-field unit system (`fields/units.ts` factor+precision registry, `UnitToggle.vue` clickable label, `NumInput` unit binding, `store.formatInUnit`). Clicking a unit now converts the shown value and adjusts dp; the store always holds SI; the selected token persists per field (`state.ui.unitTokens`, stripped from share links). Live on volume/length/area/freq/mass cells across all skins (Original entered + calculated; Modern/Classic entered via shared BoxPanel/PRPanel/DriverEditorModal). `[ui]`
- [ ] **P3** Temperature & pressure unit toggles (Advanced tab) still decorative — they need OFFSET conversion (K/°C/°F, Pa/kPa/atm) not just a factor, and sit on non-`NumInput` inputs. Extend `units.ts` with an offset and wire the two Advanced fields. `[ui]`

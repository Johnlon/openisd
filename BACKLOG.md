...
- [ ] **Original skin — reach full mock fidelity (must be IDENTICAL to `mock/`).** The audit mandate (`.claude/agents/arch-reviewer.md` step 3a) requires `OriginalShell.vue` to be visually and structurally identical to the mock; only the fake-state→shared-engine swap may differ. Phase 0 is a simplification (404 vs 3097 mock lines). Full gap list is in `CODE_REVIEW/ARCH_REVIEW_LOG.md`; the high-severity divergences to close:
  - **Toolbar** — port the 8 icon controls + the full ~19-item chart-type dropdown + the cursor readout (`mock/index.html:24-105`); today it's 2 text buttons + a bare select.
  - **Driver tab** — add the Placement section (num. drivers, Standard/Iso-Barik radios + wiring diagrams) and the Advanced-options column (`mock/index.html:269-315`).
  - **Signal tab** — add the "Listening place" group (Distance/Angle) (`mock/index.html:617-641`).
  - **Unit-cycling** — the `data-ug`/`cycleUnit` click-to-cycle unit labels on every field.
  - **Provenance + save bar** — the Entered/Calculated/Not-available colour convention and the Unsaved-changes / Save-Changes / Revert bar (`mock/index.html:255-266`), sourced from the real store's `stateOf()` provenance.
  - **Modals/docked panels** — the 3-mode Driver Editor (project / My Drivers / standalone), New Project wizard, Select Driver, Options, Box losses, docked Tune panel, Filter Editor.
  - **Medium** — projects-list checkbox is toggle-visibility (not delete); restore the Box-losses "Advanced→" link + Qtc row + dual-chamber resonance readouts; build the full Vents + Advanced tabs (per-chamber vent geometry incl. ABC intrachamber vent); spinner drag-to-scrub on numeric fields.
  Reuse shared components/composables where they exist (no forked logic). Cleanest route: port the mock's real body markup + `mock/style.css` wholesale, converting `onclick`→`@click` and `value`→`v-model` bound to the store, rather than re-deriving a simplified shell. `[ui]`

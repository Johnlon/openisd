...
- [ ] **DESIGN — the whole ＋Clone/Compare "Projects" concept needs investigating and designing (AI-introduced, never human-specified).** The current mechanic (`pinCompare()`, `packages/ui/src/store.ts:330` — snapshot the current design, push it onto `state.compare`, render it as an extra trace + a "Projects" list row) was invented by the AI; the human has not designed what a "project" / overlay / clone actually is. Questions to settle: is a Projects-list row a real openable project (WinISD semantics — several projects open at once, one selected, each independently editable) or a dead snapshot trace? Should clones be selectable / renamable / editable / saveable? How does an opened `.openisd.json` project relate to the list (see the open-project bug below)? What do the row checkbox and ✕ each mean under the chosen model? Are compare snapshots part of a saved project (`serialize()` in `packages/ui/src/utils/persist.ts:25` writes them; `importFile()` drops them)? Deliverable: a written design (a `UI_UX_DESIGN.md` section) agreed with the human BEFORE building further Compare features. `[design]`

- [ ] **Bug — "Open…" of a saved `.openisd.json` project appears to do nothing: the Projects list never shows it, and its meta/overlays are dropped.** Root cause (investigated 2026-07-22) is threefold:
  - `importFile()` (`packages/ui/src/composables/useDesignIO.ts:78-98`) restores only `driver`/`box`/`P`/`graphs` — it silently DROPS the `project` meta (name/creator/created/description) and the `compare` overlays that `serialize()` (`packages/ui/src/utils/persist.ts:25`) writes into every save, and never calls `markProjectSaved()` (so a freshly-opened project starts "modified").
  - The hash/local restore path `applyState()` (`packages/ui/src/App.vue:29-36`) restores `project` but also drops `compare` — two divergent restore implementations of the same format.
  - The Original skin's Projects list (`packages/ui/src/shells/original/OriginalShell.vue` `.projects-list`) labels the current row `driverShort(driverRaw)` only; `state.project.name` appears solely in the title bar — so opening a project that uses the same driver is visually a no-op.
    Fix options: **(a) recommended** — one restore path: `importFile` delegates to a shared `applyState` extended to restore `compare` and call `markProjectSaved()`, and the current Projects row shows `state.project.name` (falling back to the driver name); **(b) minimal** — `importFile` additionally applies `o.project` + `o.compare`. Awaiting the human's pick; the list's deeper behaviour belongs to the Clone/Compare design item above. `[ui]`

- [x] [x] **Original skin layout ergonomics + chart level-lines** (human request 2026-07-22). ＋ Compare renamed **＋ Clone/Compare** in all three shells (`OriginalShell.vue`, `ClassicShell.vue`, `GraphToolbar.vue`); the overlay ✕ remove moved to the LEFT of the overlay name and is visible at rest (was a right-side hover-reveal); the left panel and the bottom section are splitter-resizable and collapsible (`.split-v`/`.split-h` + toggles); a chart maximise/restore button (`.chart-max-btn`) grows the graph over the whole main area while the toolbar stays usable; every chart draws a horizontal level line where the cursor's frequency crosses the current design's curve — one on hover, one per selection edge during a drag-select (`packages/ui/src/utils/canvas.ts`). Layout prefs live in `state.ui.original*` (persisted locally, stripped from share links — `persist.ts`). Tests: `packages/ui/test/original-layout.browser.spec.ts` (9 cases) + `persist.test.ts` strip assertions. `[ui]`
...
- [x] **P3** Options dialog (all three skins) → **done**. Recreates WinISD's real "Options" modal (`docs/winisd/info/options_general.md` — opened via the wrench/tools toolbar icon, previously a disabled placeholder on Original/Classic). General tab's "Units" group has WinISD's own "Reset to Metric (l, mm, …)" button, wired to `store.resetUnitTokens()` — clears `state.ui.unitTokens`, reverting every field to its own default display unit in one click, never touching the stored (SI) design. WinISD's Username + Environment fields (General tab) and the whole Plot Window tab (chart colours, per-chart axis limits) are real WinISD controls not yet built — shown as an explicit "not yet in OpenISD" stub, not fabricated. `[ui]`
...
- [ ] **P1** Share link should also carry the **cursor/marker state** — the selected graph point(s) the sender had placed (`state.cursorF`/`state.pinnedF`/`state.cursorLocked`; "both cursors if two active" — the live hover cursor plus a locked/pinned second marker), so a recipient sees the exact same annotated reading the sender was pointing at, not just the same design. These aren't in `state.ui` today and `serialize()` doesn't carry them at all — needs a new field in `SerializedState` (or `UiState`), included by `stateToUrl` (kept out of `saveLocal`'s exclusion set the same way tab/chart already are). Selected tab and selected chart are **already** carried (`originalProjectTab`/`originalChartTab`/`classicProjectTab`/`classicChartTab`, kept by `stateToUrl` since the skin/tab fix above) — only cursor state is the gap. `[unit]` `[ui]`

...
- [ ] **Session-context persistence & sharing — two requirements over ONE "session context" object.** Both requirements serialize the same _session context_: the design PLUS the view/UI context that defines "what the user is looking at". They differ only in the sink (localStorage vs URL) and in which fields are device-local. `[ui]`
  ...
  **Session context taxonomy** (what must be captured):
  - **Design** — driver (full ADT JSON), box type, params `P` (incl. filters), comparison traces. _Persisted today_ (`serialize()`, `persist.ts`).
  - **View context** — skin, active project tab (`state.ui.originalProjectTab`), selected chart (`state.ui.originalChartTab`), and per-chart Y-zoom (`state.yRanges`). Tab + chart are _persisted today_ (in `state.ui`); **`yRanges` is NOT** (it is a top-level `state` field, absent from `serialize()`).
  - **Project meta** — name/creator/created/description. _Persisted today_.
  - **Transient (NOT persisted, by design)** — an open Tune/edit dialog and its uncommitted what-if/edit buffer, live hover cursor/drag. Per `STATE_MODEL.md` only committed (ground/modified) state is durable; a refresh reverts any uncommitted what-if/edit to committed.

  **R1 — Refresh fidelity (PRIMARY).** A browser refresh (F5) must repaint the _exact_ prior context — zero perceptible change. This is purely a **local-persistence completeness** problem: save the full session context to localStorage on change and restore it on load. **It does NOT involve the URL and therefore has zero impact on any skin's address bar.** _Gap to close:_ `yRanges` (and any other view state found missing) must be added to the persisted context; verify tab/chart/zoom all round-trip via a `[ui]` refresh test. Uncommitted what-if/edit is intentionally dropped on refresh (reverts to committed).

  **R2 — Full-context share link (SECONDARY, beneficial).** An explicit "Copy share link" action encodes the SAME session context (design + comparisons + project meta + **shareable** view context: active tab, selected chart, Y-zoom) into a `#s=…` URL, so opening it reproduces the page for another person. Restored by `loadFromHash()` on open. This is a link _sharing the project WITH its UI context_, richer than today's `shareLink()` which encodes design-only and strips `ui`. Device-local fields (skin) stay excluded so the recipient keeps their own skin.
  ...
  **Decisions to confirm before building:** (a) is chart Y-zoom (`yRanges`) part of the context to persist/share — yes assumed; (b) confirm uncommitted what-if/edit is discarded on refresh (yes, per STATE_MODEL); (c) share link excludes skin but includes tab/chart/zoom — confirm.

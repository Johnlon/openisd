...
- **Cursor simplified to a plain marker.** Dropped the lock toggle and clear (✕) buttons and the lock concept: hover always reads any point, and click / the ◄ ► step arrows / the Hz box place a marker that shows when the pointer is off the graphs. Right-click still snaps the marker to the nearest peak/trough.

...
# npm's auto-prebuild hook (bundle-drivers-if-changed + version-info + lint) is replicated
# explicitly below, minus lint, to avoid linting twice (once here via `check`, once via prebuild).
build:

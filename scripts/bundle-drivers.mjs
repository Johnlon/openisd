...
function main() {
  ...
  const bundle = {
    ...
    sources: [],
    // Passive radiators as their OWN collection, not rows inside `sources[].files`: the two
  ...
  for (const [key, src] of Object.entries(sources)) {
    ...
    console.log(`  → ${files.length} drivers + ${bundle.passiveRadiators.length} passive radiators bundled from ${key}`);

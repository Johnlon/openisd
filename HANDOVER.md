...
| R23 | **The projection is a SEPARATE PACKAGE — `@openisd/projection`.** `winisd_tools` embeds V8 and loads ONE bundled JS file; it cannot load a Vue app. The package depends on `@openisd/model` + `@openisd/engine` and on nothing touching DOM, `window`, network or filesystem. SPA and pipeline share the model and engine; each brings its own shell (Vue / V8). | SPECIFIED. Not built. |
| R4 | **OpenISD OWNS `openisd.yml` AND `.wdr`; `winisd_tools` owns `driver.yml`.** The Python pipeline scrapes and writes `driver.yml` into `winisd_drivers/db`, reads it, and hands the TEXT to an OpenISD-exposed utility, which returns the `openisd.yml` and `.wdr` TEXT. **`winisd_tools` writes both files; the utility touches no files at all** — and the utility is its own package (`@openisd/projection`, R23), not the browser app — it is handed no path and opens nothing, which is why the contract is string-in/string-out. The dependency runs BOTH WAYS on purpose — `winisd_tools` knows OpenISD's interface, and **OpenISD reads `driver.yml`, a schema `winisd_tools` owns**. That buys ONE implementation of each transform instead of one per language. The Python `openisd.yml` model is to be DELETED. | Doc corrected; Python deletion NOT started |
...
| L9 | **`docs/design/WINISD_SCHEMA.md` full rewrite from evidence** was ordered by the human in an earlier session ("Stop the agent, full rewrite from evidence") and never dispatched. Known problems: §2's canonical-field-order table is stale, §7 "Common mistakes" mixes format facts with obsolete workflow advice, and all `drivers/matt/` citations must go. | `docs/design/WINISD_SCHEMA.md` |

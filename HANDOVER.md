...
| R4 | **OpenISD OWNS `openisd.yml` AND `.wdr`; `winisd_tools` owns `driver.yml`.** The Python pipeline scrapes and writes `driver.yml` into `winisd_drivers/db`, then calls an OpenISD-exposed utility with it; OpenISD writes `openisd.yml` and `.wdr` beside it. The dependency runs BOTH WAYS on purpose — `winisd_tools` knows OpenISD's interface, and **OpenISD reads `driver.yml`, a schema `winisd_tools` owns**. That buys ONE implementation of each transform instead of one per language. The Python `openisd.yml` model is to be DELETED. | Doc corrected; Python deletion NOT started |

...
Successor to [PLAN_DRIVER_ADT.md](PLAN_DRIVER_ADT.md) (that plan is DONE — it built today's
...
existing `docs/DRIVER_ADT_DESIGN.md` describes the class AD-8 retires — not repeated here.
...
**Blocking gap, per AD-8 itself**: `SPL` (a printed sensitivity figure) is a canonical spec
field with no equivalent anywhere in today's engine types. Already present in `SpecSection`
here (`openisdRecord.ts:166`) but nothing downstream consumes it yet — must be resolved before
"no data loss" is true, not before Phase 1 can start.

**Also open, per AD-8, not this plan's to resolve**: for a `manual`-origin entry, what happens
to `read_precision`/`actual_reading` (no printed form to echo). Phase 1 needs an answer before
`enter()`'s write shape is locked — carry it forward as a blocking question, not invented here.

...
5. Never hand-edit generated driver data. The federated collections are
   regenerated by the sibling `winisd_tools` pipeline — fix it there, not here.
   `drivers/matt/` in this repo is human-curated, not generated, and is the one
   exception edited directly under human authorisation. This is the single most
   important rule.
...
Decision rights over the data pipeline itself (fetch/extract rules, schema
mirroring, `dq_check.py`) live in the sibling `winisd_tools` repo's own
...
### 5. Data pipeline loop (external)

Driver data ingestion is **regenerable data**, not source, and happens entirely
in the sibling [`winisd_tools`](../winisd_tools) repo (fetch → schema-validate →
`dq_check.py` → `_problems.log` → human review → fix → re-run), writing into
[`winisd_drivers`](../winisd_drivers). This repo only consumes the result — see
`drivers/README.md` for how federation works. The one exception is
`drivers/matt/` here, which is human-curated, not generated, and is restored
from archive via `scripts/restore_matt_from_archive.py` under its own approved
transformation rules — never patched ad hoc (`CLAUDE.md`: "No normalisation or
fix-it scripts").
...
| No fix-it scripts        | No standalone script that rewrites `drivers/` data               | Review + `restore_matt_from_archive.py` invariant |
...
- **One source of truth per concern.** One canonical schema doc, one roadmap
  file. Duplication is where drift and wasted review time live.
